...
import { runDefaultInit } from "../../src/commands/init-default.js";

// These tests cover the byte-equivalent behavior of the legacy
// `forge init` (no flags) flow. Migrated from init.test.ts in
// b5-1-init-wizard Phase 2 (FR-IW-003 + ADR-006). NFR-IW-004
// pins the legacy semantics.

describe("runDefaultInit (integration)", () => {
  let source: string;
  let target: string;
  ...
  it("never copies .claude/settings.local.json", async () => {
    ...
    await runDefaultInit({ sourceDir: source, targetDir: target, force: true });
  ...
  it("scaffolds .forge/product/* from templates, never from source product/", async () => {
    ...
    await runDefaultInit({ sourceDir: source, targetDir: target, force: false });
  ...
  it("is idempotent: second run copies zero additional files without --force", async () => {
    ...
    const first = await runDefaultInit({
      sourceDir: source,
      targetDir: target,
      force: false,
    });
    const second = await runDefaultInit({
      sourceDir: source,
      targetDir: target,
      force: false,
    });
