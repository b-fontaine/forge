// Forge — `forge init --archetype default` scaffolder.
// Extracted in b5-1-init-wizard (FR-IW-003 + ADR-006). Behavior
// is byte-equivalent to the pre-b5.1 `initCommand` — preserves
// NFR-IW-004 backwards compat for CI scripts invoking the legacy
// `forge init` flow.
...
export interface DefaultInitOptions {
  sourceDir: string;
  targetDir: string;
  force: boolean;
}

export interface DefaultInitResult {
  ops: Op[];
  errors: string[];
}
...
export async function runDefaultInit(
  opts: DefaultInitOptions,
): Promise<DefaultInitResult> {
  const errors: string[] = [];
