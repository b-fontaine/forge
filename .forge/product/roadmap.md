...
| **T5** (2026-Q3) | **Phase 1 Done 2026-05-06** via `t5-connect-codegen` (PR #3). **J.7 Done 2026-05-08** via `j7-validate-standards-yaml` (PR #4 merged). **OTel + OBI + Coroot stack templates Done 2026-05-10** via `t5-otel-stack` (PR #5 merged — 14/14 tests, +6 PASS verify.sh, snapshot 520 KB, `observability.yaml` 1.0.0 → 1.1.0). **J.8 Done 2026-05-10** via `j8-janus-rules` — Janus forbidden-archetypes refusal (J8-RULE-001..003, exit 3 + `[REFUSAL: ...]` stderr), `--eu-tier T1\|T2\|T3` flag plumbing (validated against `compliance-tier.schema.json`, env-var `FORGE_EU_TIER` ABI to wrappers, `.forge-tier` ledger), T3 enforcement in fsm wrapper (refuses Datadog + signoz.io + cloud-managed identity), CycloneDX 1.5 SBOM via `bin/forge-sbom.sh` (handcraft Python inline, recursive lockfile detection, `SOURCE_DATE_EPOCH`-deterministic, NIS2/DORA/CRA-aligned), 2 new standards (`global/janus-orchestration-rules.md` + `global/sbom-policy.md`) ; 20/20 tests, +6 PASS verify.sh, smoke 74 components on flagship example. **Still pending in T5** : K.3 (Demeter agent), I.2–I.6 (compliance docs + workflow + AI Act / NIS2 / DORA / CRA artefacts beyond the SBOM piece shipped here), traceparent W3C E2E validation (deferred with the L2 fixtures to T6). | `L` | Low — additive only, fully reversible |
