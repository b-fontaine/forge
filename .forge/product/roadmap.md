...
|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|
| **In Progress** | **Archetype `full-stack-monorepo`** (Flutter + Rust + Infra) with CLAUDE.md scoping, protos as single source of truth via buf, multi-layer change workflow, and the `Janus` cross-layer orchestrator agent. **Foundations delivered 2026-04-21 via `b1-foundations`** (schema, 3 new standards, scoped commits, monorepo versioning, validator). Remaining: `b1-scaffolder` (wizard + templates), `b1-workflow` (Janus + multi-root linters), `b1-delivery` (CI + `forge upgrade`). | Audit Module B.1 — flagship archetype, highest differentiation vs. alternatives                   |
