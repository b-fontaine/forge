...
|----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------|---------------------------------------------------------|
| **Done**       | **Methodology ratification — T4 in one shot.** All five T4 methodology pre-requisites of `docs/new-archetypes-plan.md` §2 (P-1 through P-4) **delivered as a single consolidated change** rather than ten separate ADR archives — the audit trail is preserved through 35 ADDED FRs (`FR-T4-ADR-001..010` + `FR-T4-STD-001..006` + `FR-T4-LC-001..005` + `FR-T4-SCH-001..002` + `FR-T4-LNT-001..002` + `FR-T4-TST-001..005` + `FR-T4-IDX-001..002` + `FR-T4-DSP-001` + `FR-T4-DOC-001..003`) + 8 NFRs in `.forge/specs/adr-ratification.md`. Six versioned `.forge/standards/*.yaml` shipped at v1.0.0 (`transport`, `state-management`, `observability`, `orchestration`, `identity`, `persistence`). 12-month review cycle wired (`global/standards-lifecycle.md` + `REVIEW.md` ledger). Two new JSON schemas (`compliance-tier.schema.json` T1/T2/T3 + `archetype.schema.json` v2 with the 5-archetype enum + `mobile-only` legacy alias). Constitution v1.1.0 unchanged ; ADR-006 amendment-versioning precedent reused. **P-5 (Hera 9 → 5 sub-agents) deferred** — flagged as `opinion d'architecte non sourcée`, awaits user decision before T6. | P-1, P-2, P-3, P-4, I.1, J.1–J.6 of `new-archetypes-plan.md` | Archived 2026-05-04 via `t4-adr-ratification` (PR #2)   |
| In progress    | **Connect codegen on the flagship template (T5 Phase 1, additive).** Branch `t5-connect-codegen` (planned, RED Phase 1 active). Adds `protoc-gen-connect-{go,es,dart-community}` to `templates/full-stack-monorepo/1.0.0/proto/buf.gen.yaml` ; bumps `transport.yaml` 1.0.0 → 1.1.0 with codegen pinning + REVIEW.md `Updated` entry (4 tests GREEN already) ; pivots to **`connectrpc-build` via `build.rs`** (Option 2) after T-BUF investigation revealed the BSR remote plugin is not yet shipped — the codebase convention is "remote plugins only", so `connectrpc-build` is consumed as a build-dependency in `backend/crates/grpc-api/Cargo.toml.tmpl` instead. Pins `connectrpc = "=0.3.3"` (Anthropic OSS, Apache-2.0, 6 558 conformance tests, MSRV 1.88). Adds parallel axum Connect route ; tonic-build untouched (ADR-004). New demo `demo-005-connect-greeting` in `examples/forge-fsm-example/`. Test harness `t5.test.sh` skeleton with 25 L1 + 5 L2 stubs.                                                                                                                                                                                                                                                                                                                                              | T5 / Phase 1 ARCH item #1 | WIP on branch `t5-connect-codegen` (commits at `281135b`) |
...
|---------|------------------------------------------------------------------------------------------------------|---------|----------|
| **T4** (2026-Q3 early) | **Done 2026-05-04 via `t4-adr-ratification`** : P-1 (10 ADRs consolidated as 1 archived change rather than 10 — Article V audit trail preserved through `FR-T4-ADR-001..010`) ; P-2 / J.1–J.6 (six `.forge/standards/*.yaml` v1.0.0 shipped) ; P-3 (`global/standards-lifecycle.md` + `REVIEW.md` ledger) ; P-4 / I.1 (`compliance-tier.schema.json` + `archetype.schema.json` v2). **P-5 (Hera 9 → 5) still pending — user decision before T6.** | `M` (delivered) | — |
| **T5** (2026-Q3) | **In progress on branch `t5-connect-codegen`** (planned, Phase 1 RED) — Connect codegen additive : buf.gen.yaml extended with `protoc-gen-connect-{go,es,dart-community}` + Rust path via `connectrpc-build` build-dependency (`connectrpc = "=0.3.3"`), `transport.yaml` 1.0.0 → 1.1.0, demo-005-connect-greeting, parallel axum Connect route, tonic-build untouched. **Still pending in T5** : OTel + OBI + Coroot stack templates, J.7 (`validate-standards-yaml.sh`), J.8 (Janus forbidden-list rules), K.3 (Demeter agent), I.2–I.6 (compliance docs + workflow + AI Act / NIS2 / DORA / CRA artefacts), traceparent W3C E2E validation. | `L` | Low — additive only, fully reversible |
...
| P-5    | Refactor Hera 9 → 5 sub-agents (UI builder, State Bloc, Animation, Test BDD, OTel client). **Requires user decision before T6** — flagged as `opinion d'architecte non sourcée`, breaks any orchestration that invokes the 9 sub-agents by name. | ARCH §9.3                       | `M`    |
...
| **ADR ratification (architecture target)**                 | 2026-09-30  | 10 ADRs of `docs/ARCHITECTURE-TARGET.md` archived as `.forge/changes/` ; six versioned `.forge/standards/*.yaml` ; compliance T1/T2/T3 schemas ; Demeter agent ; Phase 1 ARCH (OTel+Connect additive) shipped                                                       | **Partial (2026-05-04)** : T4 methodology done via `t4-adr-ratification` (consolidated change). T5 Connect codegen in progress on branch `t5-connect-codegen`. Demeter agent + OTel/OBI/Coroot stack still pending. |
