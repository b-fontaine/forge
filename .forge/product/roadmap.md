...
| **T5** (2026-Q3) | **Phase 1 Done 2026-05-06** via `t5-connect-codegen` (PR #3) — Connect codegen additive shipped. **J.7 Done 2026-05-08** via `j7-validate-standards-yaml` — `bin/validate-standards-yaml.sh` deterministic linter + JSON Schema Draft 2020-12 + `verify.sh` integration ; 21/21 tests GREEN, live-tree perf 122 ms (NFR-J7-001 budget 2 s). **Still pending in T5** : OTel + OBI + Coroot stack templates, J.8 (Janus forbidden-list rules), K.3 (Demeter agent), I.2–I.6 (compliance docs + workflow + AI Act / NIS2 / DORA / CRA artefacts), traceparent W3C E2E validation (deferred with the L2 fixtures to T6). | `L` | Low — additive only, fully reversible |
