...
| **T5** (2026-Q3) | **Phase 1 Done 2026-05-06** via `t5-connect-codegen` (PR #3). **J.7 Done 2026-05-08** via `j7-validate-standards-yaml` (PR #4 merged 2026-05-08 — 21/21 tests, +7 PASS verify.sh, perf 122 ms). **OTel + OBI + Coroot stack templates Done 2026-05-10** via `t5-otel-stack` — `grafana/beyla:2.0.1` DaemonSet (unprivileged-with-capabilities + Aegis annotation) + `coroot/coroot:1.4.4` Deployment + `processors.probabilistic_sampler` env-tier overlays (prod 10/staging 100/dev 100) + `observability.yaml` 1.0.0 → 1.1.0 ; 14/14 tests, +6 PASS verify.sh, snapshot 520 KB. **Still pending in T5** : J.8 (Janus forbidden-list rules), K.3 (Demeter agent), I.2–I.6 (compliance docs + workflow + AI Act / NIS2 / DORA / CRA artefacts), traceparent W3C E2E validation (deferred with the L2 fixtures to T6). | `L` | Low — additive only, fully reversible |
...
| **ADR ratification (architecture target)**                 | 2026-09-30  | 10 ADRs of `docs/ARCHITECTURE-TARGET.md` archived as `.forge/changes/` ; six versioned `.forge/standards/*.yaml` ; compliance T1/T2/T3 schemas ; Demeter agent ; Phase 1 ARCH (OTel+Connect additive) shipped                                                       | **Partial (2026-05-04 / 06 / 08 / 10)** : T4 methodology done via `t4-adr-ratification` (PR #2). T5 Phase 1 Connect codegen archived 2026-05-06 via `t5-connect-codegen` (PR #3). J.7 standards YAML automated enforcement archived 2026-05-08 via `j7-validate-standards-yaml` (PR #4 merged). T5 OTel + OBI + Coroot stack templates archived 2026-05-10 via `t5-otel-stack`. Demeter agent (K.3) + J.8 (Janus rules) + I.2–I.6 (compliance docs) still pending. |
