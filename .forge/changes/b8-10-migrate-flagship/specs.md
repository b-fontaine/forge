# Specifications: b8-10-migrate-flagship

<!-- Status: specified -->
<!-- Schema: default -->
<!-- Audit: B.8.10 (docs/new-archetypes-plan.md §4.2 B.8.10 — flagship migration
     script; ARCHITECTURE-TARGET §11 4-phase plan). GROUND-TRUTH NOTE (Article
     III.4): (1) the exit-7 [NEEDS MIGRATION: from 1.0.0 to 2.0.0] abort is
     ALREADY BUILT in forge-upgrade.sh (_a7_check_version_compat); B.8.10 fills
     the OTHER side — the orchestration script the abort points adopters to +
     docs/MIGRATIONS.md 1.0.0→2.0.0 section; (2) Phase 2 has NO DBOS leg —
     B8O cancelled Temporal→DBOS for Rust (ADR-B8O-001); Temporal RETAINED;
     the script applies ONLY the additive-first deltas; (3) ADDITIVE-ONLY —
     script MUST NOT remove Kong/Temporal/REST-bridge (B.8.14); (4) rollback
     target is the byte-frozen B.8.2 snapshot, NEVER rebuilt; (5) pure tooling
     — no standard bump. -->
...
**Governing articles**: III.1/III.2 (specs before code — no script before
specs.md + design.md + tasks.md), III.4 (Anti-Hallucination — overlay
engine internals, exact CLI wiring, and ledger marker are design-time ADRs;
...
- **The scaffold-manifest fields.** `examples/forge-fsm-example/.forge/
  scaffold-manifest.yaml` (observed): `archetype: full-stack-monorepo`,
...
- **`_a7_append_upgrade_history` shape (observed, `forge-upgrade.sh:144-187`).**
  ...
  (`project_name`, `reverse_domain`, `root_module`) are NOT touched.
...
- **Harness shape (observed, `b8-9.test.sh`).** `--level` flag; `source
  ...
  Docker; L2 opt-in env-gate `FORGE_B8_1_DOCKER=1` pattern (b8-1.test.sh).
...
- **Overlay engine is a design decision (Q-004 → ADR-B810-001).** Whether the
  script delegates file-merge to `forge-upgrade.sh`'s 3-way merge machinery
  or renders directly is open. Lean: delegate where possible. This spec DOES
  NOT fabricate `forge-upgrade.sh` internal function call sequences beyond
...
Q-002 / ADR-B810-002 for final values), Determinism (SOURCE_DATE_EPOCH
where a timestamp is emitted), and Pattern (bash-thin + Python 3 inline,
mirrors `bin/forge-sbom.sh` / `.forge/scripts/compliance/bundle.sh`).
...
- `--rollback`: restore from the frozen 1.0.0 snapshot (mutually exclusive
  with `--phase`; Q-002 → ADR-B810-002 resolves the exact interaction).
...
##### FR-B810-005 — Exit-code envelope: 0/1/2/7 (lean, Q-002)
The script MUST use the exit-code envelope:
- `0` — success (migration applied or dry-run complete).
- `1` — migration error (overlay failure, ledger write failure, etc.).
...
The exact mapping is resolved at `/forge:design` (Q-002 → ADR-B810-002) and
the values above are the **lean**. The script MUST exit with the correct code
for each failure mode. Testable: invoke with a non-1.0.0 target → exit 7.
...
overlay path are a design-time decision (Q-004 → ADR-B810-001). This FR
mandates the idempotent contract; the mechanism is left to design. Testable:
run Phase 1 twice on the same target — second run produces no diff.
...
The exact file set per delta is a design-time decision (Q-004 → ADR-B810-001).
...
##### FR-B810-033 — Merge delegates to forge-upgrade.sh machinery where possible (Q-004 lean)
The 3-way file merge for the Phase 2 overlays SHOULD delegate to the
`forge-upgrade.sh` merge machinery (`_a7_classify` / `_a7_three_way_merge` /
`_a7_resolve_owned_paths`) where possible, selecting the 2.0.0 template set
as the merge RIGHT. The exact delegation mechanism (shell-out vs source-and-
call vs dedicated render path) is resolved at `/forge:design` (Q-004 →
ADR-B810-001). This FR mandates that a SECOND merge engine is NOT introduced
if delegation is feasible. The anti-hallucination constraint: this spec DOES
NOT assert which specific `_a7_*` functions are called or how the 2.0.0
template set is located — that is ADR-B810-001's decision. Testable: the
script body MUST NOT duplicate `git merge-file` invocation logic that already
exists in `forge-upgrade.sh` unless ADR-B810-001 records evidence that
delegation is infeasible.

##### FR-B810-034 — Canary-by-route documented, not auto-wired (Q-005 lean)
...
manual step. The exact documentation form is resolved at `/forge:design`
(Q-005 → ADR-B810-005). Envoy SecurityPolicy/JWT OIDC wiring is B.8.12.
...
directory (or equivalent). The exact scope of the restore (full tree vs
overlay-only) is a design-time decision (Q-002 → ADR-B810-002). This FR
mandates that the rollback source is the frozen snapshot, not a newly generated
snapshot. Testable: `--rollback` on a Phase-2-applied fixture; after rollback,
the target matches the frozen snapshot sha256.
...
(g) the invocation lean (Q-001 → ADR-B810-003; doc-only invocation for
...
##### FR-B810-053 — CLI surface per Q-001 lean (doc-only for B.8.10)
...
wired into commander is deferred to B.8.14/B.8.15 unless ADR-B810-003 at
design rules it trivial. This spec does NOT fabricate any TS commander
registration. Testable: MIGRATIONS.md contains `bash bin/forge-migrate-flagship.sh`
as the invocation form.
...
it from a standard `forge upgrade` history entry (Q-003 lean →
ADR-B810-004). The exact field placement within the entry is resolved at
`/forge:design` (ADR-B810-004). Testable: after a Phase 2 apply, the manifest
`upgrade_history` list has one new entry with `kind: flagship-migration`.
...
##### FR-B810-078 — Harness includes b8-2 / b8-3 / b8-9 coupling guards + L2 opt-in
...
Independent review is required again before `/forge:archive`.
...
- **ADR-B810-001 — Overlay mechanism.** Open (Q-004). Lean: delegate file-merge
  to `forge-upgrade.sh`'s `_a7_classify` / `_a7_three_way_merge` / snapshot
  machinery where possible; the migration script selects the 2.0.0 template
  set as the merge RIGHT and orchestrates phases + preconditions. How the 2.0.0
  template set is located (path convention, env variable, forge-root detection)
  is the core design question. Do NOT introduce a second merge engine if
  delegation is feasible.
- **ADR-B810-002 — Exit-code envelope + flags.** Open (Q-002). Lean: `0/1/2/7`
  (7 = precondition mirroring A.7's exit-7); `--dry-run` default-safe; `--phase
  <0|1|2|all>`; `--force`; `--rollback`; `--target`; `--help`. Interaction
  between `--rollback` and `--phase` (mutually exclusive vs `--rollback`
  overrides) resolved at design.
- **ADR-B810-003 — CLI surface (Q-001).** Open. Lean: doc-only invocation
  in MIGRATIONS.md for B.8.10 (`bash bin/forge-migrate-flagship.sh`); a
  `forge migrate-flagship` TS subcommand deferred to B.8.14/B.8.15 unless
  trivial at design time.
- **ADR-B810-004 — Ledger (Q-003).** Open. Lean: reuse `upgrade_history`
  (single ledger, A.7 shape) with a `kind: flagship-migration` marker rather
  than a parallel `migration_history` block.
- **ADR-B810-005 — Phase 2 canary (Q-005).** Open. Lean: document-only —
  emit canary-by-route guidance in MIGRATIONS.md as a manual adopter step;
  wiring is B.8.12 territory.
...
- **Overlay engine internals NOT fabricated.** The exact sequence of
  `_a7_*` function calls, how the 2.0.0 template set is located as the merge
  RIGHT, and whether the migration script shells out to `forge-upgrade.sh` or
  sources it were open design questions. **RESOLVED at /forge:design
  (Q-004 → ADR-B810-001): the script `source`s `bin/forge-upgrade.sh`** (safe —
  ...
  against the 2.0.0 template-set as the merge RIGHT, deliberately NEVER calling
  `_a7_check_version_compat` (that guard is what delegated here). Only the
  function signatures and docblock comments quoted directly from
  `bin/forge-upgrade.sh` (as observed) are cited as facts.
- **Exit-code values.** The `0/1/2/7` envelope was the Q-002 lean.
  **RESOLVED at /forge:design (Q-002 → ADR-B810-002): aligned to A.7's actual
  envelope `0/2/5/7/8`** (`1` is unused by A.7; conflicts surface as exit-8).
- **Ledger `kind` marker.** **RESOLVED at /forge:design (Q-003 → ADR-B810-004):
  ...
  (`_b810_tag_last_history_kind`) — `forge-upgrade.sh` is NOT edited (owned
  file); identity fields stay frozen, append-only.**
- **No-DBOS + additive-only + frozen-snapshot constraints are HARD (testable).**
  FR-B810-032 (no DBOS), FR-B810-031 (no removal), and FR-B810-041 (no
  snapshot overwrite) are grounded in `2.0.0.yaml` `cancelled: true` (observed),
  the proposal Ground-Truth, and the B.8.2 freeze (observed sha256 file).
  These are not design-open; they are unconditional invariants.
- **`_a7_append_upgrade_history` signature quoted from observed source.**
  The function name, parameter order, and Python inline block shape are quoted
  from `forge-upgrade.sh:144-187` (direct re-read). No internal function name
  or Python block beyond what is observable in that file is fabricated.
- **sha256 digest is an observed fact.** The value
  `8d439b942bf81dbcc103e010d946504035dd410f613b31f673d7d691c3224ca9`
  is read directly from `.forge/scaffold-snapshots/full-stack-monorepo/1.0.0.sha256`.
  It is cited as a grounding fact for FR-B810-012; it is NOT an asserted
  design detail.
- **CLI surface Q-001 lean is doc-only.** No TS commander registration,
  no `cli/src/commands/migrate-flagship.ts` path, no wiring is fabricated.
  The doc-only lean is recorded; the decision is ADR-B810-003 at design.
- **Canary-by-route is document-only.** The script does NOT auto-wire Envoy
  canary weights. **RESOLVED at /forge:design (Q-005 → ADR-B810-005):
  document-only** — the script prints canary-by-route Kong→Envoy guidance;
  actual per-route cutover wiring is B.8.12.
- **Phase 1 detection predicate NOT fabricated.** How the script determines
  that the obs trio / Connect overlays are already present (file existence,
  manifest marker, sha256 check) is a design question → ADR-B810-001. This
  spec mandates the idempotent contract; the predicate is left to design.
- **Independent review required (NFR-B810-009).** These specs MUST pass an
  independent reviewer before `/forge:design`. Not self-approved here.

## Open Questions

Tracked in `open-questions.md`: Q-001 (CLI surface → ADR-B810-003, open,
lean doc-only), Q-002 (exit-code envelope + flag interactions → ADR-B810-002,
open, lean 0/1/2/7), Q-003 (ledger: reuse upgrade_history with kind marker
vs parallel block → ADR-B810-004, open, lean reuse), Q-004 (overlay engine:
delegate to forge-upgrade.sh machinery vs dedicated render path; how 2.0.0
template set is selected as merge RIGHT → ADR-B810-001, open, lean delegate),
Q-005 (Phase 2 canary: auto-wire vs document-only → ADR-B810-005, open,
lean document-only).
