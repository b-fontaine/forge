...
- [ ] Create `.forge/scripts/tests/b7-7.test.sh` skeleton — sources
  `_helpers.sh`, declares an empty `# MANIFEST:` block, implements
  `test_b7_7_manifest_self_consistency` as the first test, exits 0.
  [Story: FR-RAGEX-008]
- [ ] `chmod +x` the harness. [Story: FR-RAGEX-008]
- [ ] Run once → "1/1 PASS" (meta-test only). GREEN baseline.
  [Story: FR-RAGEX-008]
...
- [ ] Add `test_rag_example_tree_canonical_structure` — assert
...
- [ ] Add `test_rag_example_scaffold_manifest_complete` — parse
...
- [ ] Add `test_rag_example_readme_has_required_sections` — 4 H2
  sections + the "does NOT show" note. [Story: FR-RAGEX-002] [P]
- [ ] Add `test_examples_meta_readme_lists_rag_example` — grep
  `examples/README.md` for a `forge-rag-example` + `ai-native-rag` row.
  [Story: FR-RAGEX-003] [P]
- [ ] Add `test_rag_archetype_still_candidate` — assert
...
- [ ] Render the archetype via `overlay.sh` against the (b7-10-extended)
...
- [ ] Copy the rendered tree to `examples/forge-rag-example/`; verify
  `.forge/scaffold-manifest.yaml` is present + correct (stage candidate).
  [Story: FR-RAGEX-001]
- [ ] Drop into the tree and run `bash .forge/scripts/verify.sh` once to
  confirm its own gates pass. [REQUIRES-TOOLS: python3] [Story: FR-RAGEX-007]
- [ ] Write `examples/forge-rag-example/README.md` — 4 H2 sections; the
  ...
  [Story: FR-RAGEX-002]
- [ ] Append the `forge-rag-example` row to `examples/README.md`'s table.
  [Story: FR-RAGEX-003]
- [ ] Extend the `example` job in `.github/workflows/forge-ci.yml` with
...
- [ ] Add `"b7-7.test.sh"` to the harness loop (after `"c1.test.sh"`).
  [Story: FR-RAGEX-008]
- [ ] Run `b7-7.test.sh` → Phase 1 tests PASS (GREEN).
...
- [ ] Run all existing harnesses (incl. `c1.test.sh`) — confirm zero
  regression. [Story: backwards compatibility]
- [ ] Commit Phase 1: `feat(examples): b7-7 RAG example tree + CI gate`.
...
- [ ] Add `test_rag_demos_count_and_status` — exactly 3 demos; all
  `status: archived`. [Story: FR-RAGEX-004] [P]
- [ ] Add `test_each_rag_demo_has_five_artefacts` — proposal/specs/
  design(-per-layer)/tasks(-per-layer)/features per demo. [Story: FR-RAGEX-004] [P]
- [ ] Add `test_rag_demo_003_is_multi_layer` — parse
...
- [ ] Add `test_rag_demos_cover_ai_first_surfaces` — grep the demo specs
  for fallback / prompt-audit / embeddings markers. [Story: FR-RAGEX-005] [P]
- [ ] Add `test_rag_demos_manifest_present_and_lists_three_demos`.
  [Story: FR-RAGEX-006] [P]
- [ ] Add `test_each_rag_demo_proposal_under_size_budget` — ≤ 200 lines.
  [Story: NFR-RAGEX-004] [P]
- [ ] Add `test_rag_demos_cover_distinct_combinations` — distinct layer
  + RAG-surface combos. [Story: NFR-RAGEX-005] [P]
- [ ] Run `b7-7.test.sh` → 7 new tests FAIL (RED).

### Phase 2 — GREEN (chronological)

#### demo-001-doc-ingestion (single-layer backend, `rag/`)
- [ ] Bootstrap the change skeleton inside the RAG tree; set
  `layers: [backend]`. [Story: FR-RAGEX-004]
- [ ] Write proposal (≤ 200 lines) — document ingestion + RAG query.
- [ ] Write specs — FR-BE-* (chunking, `Embedder`, pgvector HNSW upsert,
  hybrid retrieval vector+BM25+RRF, re-rank) + XI.5 fallback + XI.6
  in-process local path. [Story: FR-RAGEX-004, FR-RAGEX-005]
- [ ] Write design — ADR(s): cucumber-rs, `Embedder` trait selection
  (tier-aware), pgvector `vector_cosine_ops`.
- [ ] Write tasks — TDD-ordered (RED chunking test → GREEN, RED
  retrieval test → GREEN). [Story: Article I]
- [ ] Implement TDD in `backend/rag/` per the rendered scaffolding.
  [REQUIRES-TOOLS: cargo]
- [ ] Write `features/doc_ingestion.feature` — ingest → query →
  grounded answer + fallback path; cucumber-rs steps. [REQUIRES-TOOLS: cargo]
- [ ] Mark tasks `[x]`, archive (status: archived, timeline populated).

#### demo-002-mcp-search-tool (single-layer backend, `mcp/`)
- [ ] Bootstrap; set `layers: [backend]`. [Story: FR-RAGEX-004]
- [ ] Write proposal (≤ 200 lines) — expose retriever as MCP `search`.
- [ ] Write specs — FR-BE-* (rmcp `#[tool_router]` `search` tool,
  schema-validated input, dual transport stdio/http, OAuth→Zitadel hook).
- [ ] Write design — ADR(s): rmcp transport feature-gating, least-priv.
- [ ] Write tasks — TDD-ordered. [Story: Article I]
- [ ] Implement TDD in `backend/mcp/`. [REQUIRES-TOOLS: cargo]
- [ ] Write `features/mcp_search.feature` — search happy + bounded
  paths. [REQUIRES-TOOLS: cargo]
- [ ] Archive.

#### demo-003-rag-query-ui (multi-layer backend + frontend, Janus, STREAMING)
- [ ] Bootstrap; set `layers: [backend, frontend]`; declare per-layer
  `designs/` + `tasks/` (FR-GL-016). [Story: FR-RAGEX-004]
- [ ] Write proposal (≤ 200 lines) — STREAMING Qwik query UI consuming
  b7-10's `RagService.QueryStream` via the gateway.
- [ ] Write specs — per-layer delta: FR-BE-* (gateway prompt-audit
...
- [ ] Write `designs/design-backend.md` + `designs/design-frontend.md`
  (Janus-orchestrated, ≥ 2 layers). [Story: FR-GL-015]
- [ ] Write `tasks/tasks-backend.md` + `tasks/tasks-frontend.md`.
- [ ] Implement TDD: backend `QueryStream` gateway path; Qwik
  `queryStream` progressive-render component + component test.
  [REQUIRES-TOOLS: cargo, node, buf]
- [ ] Write `features/rag_query_ui.feature` — streaming query happy path
  (progressive tokens) + fallbackUsed path (degrade to unary).
  [REQUIRES-TOOLS: node]
- [ ] Archive.

#### Manifest + closure
- [ ] Write `.forge/changes/MANIFEST.md` — 3 rows (id, status, summary).
  [Story: FR-RAGEX-006]
- [ ] Populate the README's `## Demo changes` section with the 3 demos.
  [Story: FR-RAGEX-002]
- [ ] Run `b7-7.test.sh` → Phase 2 tests PASS (GREEN).

### Phase 2 — REFACTOR
- [ ] Run all harnesses → zero regression.
- [ ] Run Forge-root `verify.sh` → `[skipped: examples]` covers the new
  demo paths.
- [ ] Commit Phase 2: `feat(examples): b7-7 — 3 RAG demo changes`.
...
### Phase 3 — RED
- [ ] Add `test_rag_example_tree_byte_budget` — ≤ 5 MB (excluding
  ignored paths). [Story: NFR-RAGEX-002] [P]
- [ ] Add `test_example_reference_spec_has_ragex_section_post_archive`
...
- [ ] Add L2 (opt-in) tests: `test_rag_example_tree_verify_exits_zero`,
...
- [ ] Run `b7-7.test.sh` → Phase 3 RED tests FAIL.

### Phase 3 — GREEN
- [ ] Verify byte budget holds; record the baseline in NFR-RAGEX-002.
  [Story: NFR-RAGEX-002]
- [ ] (At archive — handled by /forge:archive) append the `FR-RAGEX-*`
  + `NFR-RAGEX-*` section + the b7-7 row to
  `.forge/specs/example-reference.md`. [Story: FR-RAGEX-010]
- [ ] (At archive) merge the MODIFIED FR-CI-012 delta into
  `.forge/specs/forge-ci.md`.
- [ ] Run `b7-7.test.sh` → Phase 3 tests PASS (except the archive-gated
  spec test, which passes once status=archived).

### Phase 3 — REFACTOR
- [ ] Run all harnesses + verify.sh + constitution-linter.sh → zero
  regression.
- [ ] Run `b7-7.test.sh --require-example-tools` (L2) → PASS.
- [ ] Commit Phase 3: `feat(forge): b7-7 harness + example-reference consolidation`.

---

## Phase 4: Quality (pre-archive)
- [ ] Verify Article VII for demo-001/002: `cargo clippy --workspace
  --all-targets -- -D warnings` from inside the RAG tree.
  [REQUIRES-TOOLS: cargo]
- [ ] Verify demo-003 frontend: Qwik lint/type-check + component test.
  [REQUIRES-TOOLS: node]
- [ ] Confirm coverage ≥ 80% (Article X.1) for the demo code where
