...
- [x] Create `.forge/scripts/tests/a7.test.sh` skeleton — sources
...
- [x] Make `a7.test.sh` executable (`chmod +x`).
  [Story: FR-UP-014]
- [x] Run `bash .forge/scripts/tests/a7.test.sh` once : confirm 1/1
  PASS (just the meta-test). [Story: FR-UP-014]
- [x] Wire `a7.test.sh` into `.forge/scripts/verify.sh` Section 7 at
  L1 (alongside `c1.test.sh` and the existing 5 harnesses).
  [Story: FR-UP-014]
- [x] Wire `a7.test.sh` into `.github/workflows/forge-ci.yml`
...
- [x] Add `test_framework_owned_paths_yml_shape` to `a7.test.sh`
...
- [x] Add `test_forge_upgrade_sh_uses_find_excluding_examples` :
  ...
  [Story: FR-UP-009] [P]
- [x] Add `test_standard_upgrade_policy_has_required_sections` :
  ...
  [Story: FR-UP-010] [P]
- [x] Add `test_index_has_upgrade_policy_entry` : asserts
...
- [x] Add `test_gitignore_covers_merge_conflicts` : asserts root
  `.gitignore` contains `.merge-conflicts`. [Story: FR-UP-012] [P]
- [x] Add `test_features_upgrade_feature_present` : asserts
...
- [x] Add `test_snapshot_tarball_present_and_extractable` :
...
- [x] Add `test_snapshot_size_under_budget` : asserts the snapshot
  tarball's uncompressed size ≤ 1 MB. [Story: NFR-UP-003]
- [x] Run `a7.test.sh` once → confirm all Phase-1 RED tests FAIL.
  [Story: TDD discipline]

### Phase 1 — GREEN

- [x] Write `cli/assets/framework-owned-paths.yml` per ADR-003 :
  ...
  [Story: FR-UP-002]
- [x] Create `bin/forge-upgrade.sh` stub : sources `_helpers.sh`,
...
- [x] `chmod +x bin/forge-upgrade.sh`. [Story: FR-UP-009]
- [x] Write `.forge/standards/global/upgrade-policy.md` per ADR-010
...
- [x] Add new entry to `.forge/standards/index.yml` :
  ...
  [Story: FR-UP-011]
- [x] Append `.merge-conflicts` to root `.gitignore` with the
  documented inline comment. [Story: FR-UP-012]
- [x] Write `.forge/changes/a7-forge-upgrade/features/upgrade.feature`
...
- [x] Build the snapshot tarball for `full-stack-monorepo / 1.0.0` :
...
- [x] Run `a7.test.sh` → confirm all Phase-1 RED tests now PASS.
  [Story: TDD]

### Phase 1 — REFACTOR

- [x] Run `bash .forge/scripts/verify.sh` : confirm zero regression
  on the 6 prior harnesses + the new `a7.test.sh` L1 entry passes.
- [x] Run `bash .forge/scripts/constitution-linter.sh` : zero new
  warnings.
- [x] Run `shellcheck bin/forge-upgrade.sh bin/forge-snapshot.sh` :
  zero issues at severity `warning`. [Story: Article X.5]
- [x] Commit Phase 1 cluster : `feat(forge): a7-forge-upgrade
...
- [x] Add `test_merge_truth_table_exhaustive` : 5 fixtures (4 cells
...
- [x] Add `test_minor_patch_bumps_proceed` : fixture has manifest
  `1.0.0`, framework `1.1.0` → proceeds normally. [Story: FR-UP-006]
- [x] Add `test_upgrade_history_appended_after_run` : fixture
...
- [x] Run `a7.test.sh` → confirm Phase-2 RED tests FAIL.
  [Story: TDD]

### Phase 2 — GREEN

- [x] Implement BASE recovery in `bin/forge-upgrade.sh` :
  ...
  [Story: FR-UP-008]
- [x] Implement framework-owned-paths parsing :
...
- [x] Implement `--force` Git cleanliness gate :
...
- [x] Implement major-version migration abort :
  ...
  [Story: FR-UP-006]
- [x] Implement scaffold-manifest update :
...
- [x] Implement final summary print to stdout per FR-UP-001
  format. Exit code per the rules : 0 if no conflicts, 8 if
  conflicts without `--force`, 0 if conflicts with `--force`.
  [Story: FR-UP-001]
- [x] Implement `--dry-run` short-circuit : after computing the
  truth table, print the summary but do NOT mutate anything.
  Exit 0. [Story: FR-UP-001]
- [x] Run `a7.test.sh` → confirm Phase-2 RED tests now PASS.
  [Story: TDD]

### Phase 2 — REFACTOR

- [x] Run all 7 harnesses (foundations + scaffolder + workflow +
  delivery + g1 + c1 + a7) : zero regression.
- [x] Run `shellcheck bin/forge-upgrade.sh bin/forge-snapshot.sh`
  : zero issues at severity `warning`.
- [x] Run `bash .forge/scripts/verify.sh` + `constitution-linter.sh`
  : zero new failures.
- [x] Commit Phase 2 cluster : `feat(forge): a7-forge-upgrade
...
- [x] Add Vitest `cli/test/upgrade.test.ts` :
  ...
  [Story: FR-UP-001]
- [x] Add `test_upgrade_cli_flags_parse` to `a7.test.sh` :
  static text-grep on `cli/src/commands/upgrade.ts` asserts
  flag names + commander chain. [Story: FR-UP-001]
- [x] Add L3 opt-in test (gated on `--require-external-tools`) :
...
- [x] Run all tests → confirm Phase-3 RED tests FAIL.

### Phase 3 — GREEN

- [x] Write `cli/src/commands/upgrade.ts` :
  ...
  [Story: FR-UP-001]
- [x] Wire `upgradeCommand` into `cli/src/cli.ts` :
  - Import + register on the program (mirrors `initCommand` and
    `verifyCommand`).
  [Story: FR-UP-001]
- [x] Update `cli/package.json` `description` field if needed
...
- [x] Run `a7.test.sh` → all FRs GREEN (L1 + L2). L3 stays
  gated unless `--require-external-tools`.
- [x] *(opt-in)* Run `a7.test.sh --require-external-tools` for the
  L3 against `examples/forge-fsm-example/`. Confirm GREEN.
  [Story: FR-UP-014]

### Phase 3 — REFACTOR

- [x] Run all 7 harnesses + `npm test --prefix cli` + verify.sh
  + constitution-linter.sh : zero regression.
- [x] `shellcheck bin/forge-upgrade.sh bin/forge-snapshot.sh` :
  zero issues.
- [x] `npm run lint --prefix cli` : zero issues. [Story: Article X.5]
- [x] Update `docs/GUIDE.md` § Upgrade flow with the new
  command's user-facing usage. [Story: housekeeping]
- [x] Update `docs/CONTRIBUTING.md` § How upgrades affect your
  local Forge customizations.
- [x] Update README adoption section to mention `forge upgrade`.
- [x] Commit Phase 3 cluster : `feat(cli): a7-forge-upgrade
...
- [x] *(post-merge, opt-in)* Run a real upgrade against a fresh
...
- [x] /forge:archive merges the MODIFIED FR-GL-009 update into
...
- [x] /forge:archive updates `.forge/product/roadmap.md` :
...
- [x] /forge:archive runs all 7 harnesses + verify.sh +
