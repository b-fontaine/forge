...
- [x] **T001** Record evidence.md: pgvector image `pgvector/pgvector:0.8.2-pg17`
...
## Phase 1 — Harness RED
- [x] **T002** Author `.forge/scripts/tests/b8-5.test.sh` (12 L1 per design), run → RED
  (8 failed / 4 passed; tree + orchestration bump + dbos-deferred absent).
  [Story: FR-B85-050..056]

## Phase 2 — Datastore templates GREEN
- [x] **T003** Create `.forge/templates/archetypes/full-stack-monorepo/2.0.0/infra/postgres/`:
...
- [x] **T004** Bump `.forge/standards/orchestration.yaml` 1.0.0 → 1.1.0 (additive
...
- [x] **T005** Edit `2.0.0.yaml`: dbos-embedded component += `status: deferred` + `note:`;
  temporal→dbos migration_delta += `note:` deferred. postgres delta + persistence ref
  INTACT. Re-run b8-3 (17/17) + b8-3b (12/12) — stayed GREEN. [Story: FR-B85-006]

## Phase 5 — GREEN + Integration
- [x] **T006** Run b8-5.test.sh → 12/12 GREEN. [Story: FR-B85-*]
- [x] **T007** Register `b8-5.test.sh` one-line in forge-ci.yml harness loop (after b8-4).
  [Story: FR-B85-050]
