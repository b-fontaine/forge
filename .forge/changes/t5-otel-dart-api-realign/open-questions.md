...
Pending `/forge:design`. Default expectation : **scope to traces**,
add an explicit "Status (per Workiva, 2026-05-11)" callout in v1.1.0
documenting the metrics-alpha / logs-unimplemented reality.
...
Pending `/forge:specify`. Default expectation : **minimum-change realign**
— swap API names only, do not restructure the interceptor.
...
Pending `/forge:design`. Default expectation : **Path A** — document
the ratio-via-collector pattern in v1.1.0's Sampler section with a
cross-reference to ADR-OTEL-001.
