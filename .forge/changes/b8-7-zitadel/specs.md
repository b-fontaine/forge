# Specifications: b8-7-zitadel

<!-- Status: specified -->
<!-- Schema: default -->
<!-- Audit: B.8.7 (docs/new-archetypes-plan.md §4.2 — Zitadel templates brick;
     ratified by ADR-007 ARCHITECTURE-TARGET.md — REPLACE implicit auth → Zitadel;
     Schrems II + CLOUD Act. GROUND-TRUTH NOTE (Article III.4): the 1.0.0 flagship
     ships NO identity at all — B8-BASELINE §7 records the delta as
     `implicit auth → Zitadel (B.8.7)`; this brick INTRODUCES identity (additive),
     it does not migrate an existing IdP.) -->
...
- `identity.yaml` v1.0.0 (T.4 / J.5): `default: zitadel`, alternatives
  ...
  `linter_rule: null`) — documentation-only standard. It carries **no `versions:`
  map** today — this brick adds the first Zitadel pin source (like `gateway.yaml`).
...
- Zitadel is **AGPL**-licensed — adopter documentation must note this.
...
- All concrete versions (Helm chart, container image + registry, login-UI
  topology, bootstrap env-var names, masterkey constraints) are **verify-then-pin
  LIVE** at `/forge:design` and re-verified at `/forge:implement` — never
  fabricated here (b8-coroot + t5.3.2 lessons). See Q-001 through Q-005.
...
| **gateway.yaml (observed)** | v1.0.0 (B.8.4) — frontmatter model to mirror: `versions:` map + `pin_review_cadence:` ISO 8601 + dated expiry. |
...
##### FR-B87-002 — kustomization.yaml.tmpl lists all Zitadel resources
The subtree MUST contain `kustomization.yaml.tmpl` that lists all K8s resource
files present in the subtree as `resources:` entries. No resource file may exist
in the subtree without being listed in `kustomization.yaml.tmpl`. The
kustomization MUST carry an audit comment `# <!-- Audit: B.8.7 (b8-7-zitadel) -->`
and a `# Standard: .forge/standards/identity.yaml` reference (mirrors the
B.8.4 kustomization pattern). `commonLabels:` with `app.kubernetes.io/name`,
`app.kubernetes.io/component: identity`, and `app.kubernetes.io/part-of` MUST
be present (FR-B84-021 precedent / envoy-gateway kustomization shape).
**RESOLVED at /forge:design (ADR-B87-001 + ADR-B87-003):** The subtree is a 4-file chart-referenced hybrid — `kustomization.yaml.tmpl`, `values-forge.yaml.tmpl`, `README.md.tmpl`, `bootstrap.md.tmpl`. No raw deployment/service/configmap manifests are vendored; the chart's initJob + setupJob supply operator-grade bootstrap. Spec Delta governs chart upgrades.
...
(hybrid control-plane/data-plane split per ADR-B87-001 — vendor only workload
manifests, record install path in README), (b) the `standard: identity.yaml`
policy source reference, (c) the T1/T2/T3 compliance posture (§G6), (d) the
AGPL licensing note (§G6), (e) the Envoy→Zitadel OIDC delegation posture (§G6),
(f) the `NEVER PUT SECRETS HERE` warning (§G2). The README MUST carry an audit
comment `<!-- Audit: B.8.7 (b8-7-zitadel) -->`. Mirrors the B.8.4
`README.md.tmpl` format (header, delivery model table, install block, resources
table, coexistence note).

##### FR-B87-004 — Audit comment headers on all manifest templates
Every K8s manifest template file in the subtree MUST carry a top-of-file audit
comment in the form `# <!-- Audit: B.8.7 (b8-7-zitadel, FR-B87-NNN) -->` and
a `# Standard: .forge/standards/identity.yaml` line (mirrors the B.8.4
`gateway.yaml.tmpl` / `httproute.yaml.tmpl` / `backendtlspolicy.yaml.tmpl`
format and the obi-daemonset.yaml.tmpl Aegis precedent).
...
##### FR-B87-006 — Aegis annotations on all K8s workload manifests
Every K8s workload manifest (Deployment, DaemonSet, Job, or equivalent) in the
subtree MUST carry Forge Aegis annotations on its `metadata.annotations`:
```
...
flag (mirrors ADR-OTEL-004 / t5-otel-stack posture). Additional capabilities
required by Zitadel (if any) MUST be added explicitly with justification in the
manifest comment and reviewed per ADR-B87-004.
**RESOLVED at /forge:design (ADR-B87-004):** Zitadel requires no capabilities beyond a standard non-privileged workload. The securityContext uses `runAsNonRoot: true`, `readOnlyRootFilesystem: true`, `allowPrivilegeEscalation: false`, and `capabilities: drop: [ALL]` with no additions. Placement is top-level chart values (helm-template-verified, evidence Finding 8).
...
posture. The exact mechanism (`ZITADEL_FIRSTINSTANCE_*` env contract vs Admin-
API Job vs zitadel CLI) is resolved at `/forge:design` via live Zitadel docs
(ADR-B87-003, Q-003). This spec mandates the invariants that apply regardless
...
The README MUST document the masterkey handling policy (minimum length, generation
command, K8s Secret creation workflow) as sourced from the live Zitadel
documentation resolved at `/forge:design` (ADR-B87-003, Q-003). The spec does
NOT fabricate masterkey constraints — they are derived from live docs at design.
**RESOLVED at /forge:design (ADR-B87-003, evidence Finding 3):** Masterkey must be exactly 32 bytes (printable ASCII recommended). Generated via `openssl rand -base64 24 | tr -d '\n'` or equivalent. Injected via `masterkeySecretName` referencing a pre-created K8s Secret; the masterkey value is never placed in the values overlay file.

##### FR-B87-024 — Bootstrap mechanism shape: design-time resolution with invariants
The bootstrap mechanism (ADR-B87-003) is resolved at `/forge:design`. This spec
requires that regardless of which mechanism is chosen:
- The mechanism MUST create a root tenant and an OIDC client app without
  requiring any post-deploy manual console step (the bootstrap is automated).
- The mechanism MUST NOT require storing an admin credential in a committed file.
- The mechanism MUST document the JWT signing-key rotation cadence (Zitadel-
  managed vs operator duty) per live Zitadel docs.
**RESOLVED at /forge:design (ADR-B87-003, evidence Findings 5–6):** Bootstrap is the chart-native two-stage initJob + setupJob. The `FirstInstance` env-var contract drives the setupJob: `ZITADEL_FIRSTINSTANCE_MACHINEKEYPATH`, `ZITADEL_FIRSTINSTANCE_PATPATH`, and `ZITADEL_FIRSTINSTANCE_LOGINCLIENTPATPATH` are top-level fields under `FirstInstance:` (not org-nested). OIDC client app registration uses a post-bootstrap Management API `POST /projects/{project_id}/apps/oidc` call documented in `bootstrap.md.tmpl`. No separate Job is authored.
...
##### FR-B87-026 — Bootstrap Job carries Aegis annotations
If the bootstrap mechanism uses a K8s Job, the Job manifest MUST carry the Forge
Aegis annotations (FR-B87-006) and the minimal securityContext (FR-B87-007).
The Job MUST NOT run as root unless live Zitadel docs mandate it; if root is
required, it MUST be explicitly noted with justification in the manifest comment
and in ADR-B87-004.

##### FR-B87-027 — JWT signing-key rotation posture documented
The README (or a dedicated `key-rotation.md.tmpl` if design chooses) MUST
document: (a) that Zitadel manages JWT signing keys internally (or the operator
duty if live docs differ), (b) the default rotation cadence per live Zitadel docs
at design, (c) a pointer to the upstream Zitadel key management documentation.
The spec does NOT assert specific rotation cadence values — they are sourced live.
**RESOLVED at /forge:design (ADR-B87-003, evidence Finding 5):** JWT signing-key rotation is Zitadel-managed and fully automatic — no operator rotation duty. Rotation uses KeyConfig (6h activation / 30h expiry) and the WebKeys v2 API. The README documents this posture and links to the upstream Zitadel key management documentation.

---

#### Group 3 — Datastore wiring (FR-B87-030 → 037)

##### FR-B87-030 — Datastore decision resolved at /forge:design (ADR-B87-002, Q-002)
The datastore wiring (shared B.8.5 `fsm-db` instance with a dedicated
`zitadel` database/role vs a dedicated Postgres instance) is a design-time
decision (ADR-B87-002, Q-002). This spec mandates the invariants that apply
regardless of the chosen topology.
...
The README MUST document which Postgres major versions Zitadel supports (live-
checked at `/forge:design` against the official Zitadel documentation) and
explicitly state whether the B.8.5 `pgvector/pgvector:0.8.2-pg17` image satisfies
Zitadel's Postgres compatibility matrix (ADR-B87-002 Q-002 lean: reuse fsm-db).
The compatibility claim MUST be sourced from live Zitadel docs at design — NOT
asserted here.
**RESOLVED at /forge:design (ADR-B87-002, evidence Finding 1):** Zitadel requires PostgreSQL 14+. The B.8.5 `pgvector/pgvector:0.8.2-pg17` image satisfies this requirement (17 ≥ 14, compatibility confirmed). The shared `fsm-db` pg17 instance is reused; no dedicated Postgres instance is needed for dev posture.

##### FR-B87-034 — Dedicated database/role documented for dev posture
Whether the topology uses a shared `fsm-db` or a dedicated Postgres instance,
...
isolation). The dev compose fragment MUST reflect the chosen topology.

##### FR-B87-035 — Fragment image pin is verify-then-pin LIVE at /forge:design
If the compose fragment references a Zitadel container image, the image tag MUST
be verified live at `/forge:design` (registry identity check — docker.io vs ghcr,
the b8-coroot lesson) and re-verified at `/forge:implement`. No image tag is
fabricated in this spec.
**RESOLVED at /forge:design (ADR-B87-001, evidence Finding 4):** Canonical registry is `ghcr.io` (not docker.io). Pinned pair: `ghcr.io/zitadel/zitadel:v4.14.0` (chart 10.0.2 appVersion, manifest-verified 2026-06-02). `postgresql.enabled: false` is set; DSN injected via `ZITADEL_DATABASE_POSTGRES_DSN` secretKeyRef (evidence Finding 2).

##### FR-B87-036 — init-SQL or DB-init asset ships if required by chosen topology
If the shared `fsm-db` topology (ADR-B87-002 lean) is chosen, a database/role
init asset (SQL script or equivalent K8s init Job) MUST ship in the subtree to
create the dedicated `zitadel` database and role. The asset MUST NOT contain a
hardcoded password — password references MUST use `${ZITADEL_DB_PASSWORD}` or
equivalent env-var form. The exact form of this asset is resolved at
`/forge:design` (ADR-B87-002).
...
The v1.1.0 identity.yaml MUST add a `versions:` map (the first Zitadel pin
source for this standard, mirroring `gateway.yaml`). The map MUST contain at
minimum the Zitadel container image tag and (if the design chooses Helm) the
Zitadel Helm chart version — both verified live at `/forge:design` and
re-verified at `/forge:implement` (ADR-B87-001, Q-001). The map MUST NOT
contain fabricated values.
**RESOLVED at /forge:design (ADR-B87-001 + ADR-B87-005, evidence Finding 4):** The `versions:` map ships as: `zitadel_chart: "10.0.2"`, `zitadel: "v4.14.0"`, `zitadel_login: "v4.14.0"` (all `ghcr.io`, manifest-verified 2026-06-02). `pin_review_cadence:` is added per ADR-B87-005. Shipped in identity.yaml v1.1.0.
...
chart if applicable) are resolved at `/forge:design` based on Zitadel's upstream
release velocity. The field MUST use ISO 8601 duration format.
...
`identity.yaml v1.1.0`, dated 2026-06-02 (the B.8.7 specify date), with a one-
line description (additive: first `versions:` map + `pin_review_cadence:` —
Zitadel becomes a pin source like `gateway.yaml`). Mirrors the B.8.4
`gateway.yaml` and B.8.6 `transport.yaml` REVIEW.md precedents (Article XII
append-only ledger).
...
The updated `expires_at:` (if present) and header comment MUST reflect v1.1.0
and the B.8.7 change date.
...
is not set). No WAIVER commentary is needed for the bump itself (WAIVERs may
be needed for the Zitadel image/chart pins inside the versions block — those are
ADR-B87-001/005 territory, resolved at `/forge:design`).
...
schema is consistently annotated across delivered bricks. The exact formatting
(spacing, dash vs em-dash) is resolved at `/forge:design` by inspecting the
live B.8.4/B.8.6 annotations.
...
The exact wording is resolved at `/forge:design` but MUST convey: AGPL-3.0,
commercial licensing note, and a pointer to Zitadel's official site (AT:1089).
...
##### FR-B87-071 — Harness asserts 2.0.0/infra/zitadel/ subtree exists
The harness MUST assert that `2.0.0/infra/zitadel/kustomization.yaml.tmpl`,
`2.0.0/infra/zitadel/README.md.tmpl`, and `2.0.0/infra/zitadel/docker-compose.fragment.yml.tmpl`
exist. A missing required file is a FAIL. Additional manifest files (determined
at design, Q-001/Q-004) are also asserted as present.

##### FR-B87-072 — Harness asserts kustomization lists all resources
The harness MUST assert that `kustomization.yaml.tmpl` contains a `resources:`
block and that every `.yaml.tmpl` manifest file present in the subtree (excluding
`kustomization.yaml.tmpl` itself) appears in the `resources:` list (FR-B87-002).
The test is a grep/diff of file listing vs kustomization resources entries.
...
`forge.dev/standard: "identity.yaml@1.1.0"` appear in at least one manifest
template in the subtree (FR-B87-006). Mirrors b8-5's observability.yaml
annotation check.
...
delivered annotation comment and that `standard: identity.yaml` still resolves
(FR-B87-050/051). The `implicit-auth → zitadel` migration_delta MUST still be
present with `strategy: additive-first` (FR-B87-052).
...
##### NFR-B87-005 — Full ~45-harness suite GREEN pre-push
Before pushing, the full forge-ci harness suite (all ~45 harnesses in
...
All Zitadel version pins (image tag, Helm chart version if applicable, registry
identity) MUST be resolved from live registries at `/forge:design` (Q-001,
ADR-B87-001) and re-verified at `/forge:implement` (b8-coroot lesson — hosts
move between design and implement). No version string is fabricated at specify
phase. A version asserted without live evidence is a constitutional anti-
hallucination failure (Article III.4).
...
`/forge:design` proceeds (t5-2 self-validation lesson — "Article VIII" fabriquée
catch). Self-approval of the anti-hallucination pass (Article III.4) and of the
open-questions leanings is prohibited. The verify-then-pin items (Zitadel image
tag, Helm chart, bootstrap env-var names) are NOT resolved at specify phase.
...
lesson: gates re-run post-flip). A green run before the flip does not satisfy
...
- **ADR-B87-001 — install source + pins.** Hybrid delivery: vendor the workload
  manifests (Deployment/Service/ConfigMap + bootstrap Job — no CRDs/controller);
  record Helm chart AND container image pins live (registry identity verified).
  Lean: manifests-only (Zitadel is a plain Deployment/Service, no CRD/controller
  half). Chart AND image pins recorded in identity.yaml v1.1.0 `versions:` map.
  Registry identity (docker.io vs ghcr) resolved live at `/forge:design`.
- **ADR-B87-002 — datastore.** Lean: reuse the 2.0.0 `fsm-db` Postgres 17 with
  a dedicated `zitadel` database + role for dev; document dedicated-instance
  separation as the T2/T3 prod posture. Verify live: Zitadel's supported Postgres
  versions vs the `pgvector/pgvector:0.8.2-pg17` image.
- **ADR-B87-003 — bootstrap mechanism.** Lean: `ZITADEL_FIRSTINSTANCE_*` env
  contract for root tenant + machine user; a bootstrap Job (Admin API) for the
  OIDC client app; signing-key rotation documented as Zitadel-managed with cadence
  noted. Verify the exact contract live (Zitadel docs via Context7/official docs).
  Env-var names NOT fabricated at specify phase.
- **ADR-B87-004 — secrets posture.** Lean: masterkey + admin secrets via K8s
  Secret refs generated at deploy time, never templated; `forge.dev/aegis-audit:
  "required"` + `forge.dev/standard: "identity.yaml@1.1.0"` annotations; minimal
  securityContext (`drop: [ALL]`, no `privileged: true`).
- **ADR-B87-005 — identity.yaml versioning.** Lean: additive 1.1.0 (first
  `versions:` map — standard becomes a pin source like `gateway.yaml`); no
  `breaking_change`; enforcement stays off; `pin_review_cadence:` ISO 8601.
- **ADR-B87-006 — Envoy OIDC wiring scope.** Lean: defer actual SecurityPolicy/
  JWT filter + backend middleware to B.8.10/B.8.12 — this brick is the IdP
  infrastructure; document the delegation contract only.
...
Feature: Zitadel IdP introduced additively as the 2.0.0 identity component via versioned subtree, standard bump, and schema annotation
  ...
  Scenario: The 2.0.0 identity subtree lands additively without disturbing any frozen 1.0.0 asset
    ...
    Then the 2.0.0/infra/zitadel/ subtree exists with kustomization.yaml.tmpl, README.md.tmpl, docker-compose.fragment.yml.tmpl, and all manifest templates
    And kustomization.yaml.tmpl lists every manifest template file as a resources: entry
    ...
    And b8-7.test.sh passes all L1 checks within 2 s
  ...
  Scenario: Secrets posture holds — no secret value appears in any committed template
    ...
    And all K8s workload manifests carry forge.dev/aegis-audit: "required"
...
## Spec Delta (design-phase, ADR-B87-001 — 2026-06-02)

Q-001 live evidence **falsified the manifests-only lean** (see `design.md`
ADR-B87-001 + `open-questions.md` Q-001 Resolution + `evidence.md` Finding 4).
The chart-referenced hybrid posture (option b) eliminates the need for
`kustomization.yaml.tmpl` and changes the required file set. The following FRs
are modified by the design-phase decision; all other FRs remain in force.

##### FR-B87-002 — MODIFIED (ADR-B87-001, Q-001 Resolution)

**Previously**: the subtree MUST contain `kustomization.yaml.tmpl` listing all
K8s resources; `commonLabels:` required; kustomize-pattern audit comment required.

**Now**: the subtree uses a **chart-referenced hybrid** (helm install + Forge
values overlay). No raw K8s manifests are vendored; accordingly
`kustomization.yaml.tmpl` is NOT delivered. Instead the subtree MUST contain
`values-forge.yaml.tmpl` — a Forge Helm values overlay that carries:
...
##### FR-B87-071 — MODIFIED (ADR-B87-001, Q-001 Resolution)

**Previously**: harness MUST assert `kustomization.yaml.tmpl`, `README.md.tmpl`,
and `docker-compose.fragment.yml.tmpl` exist; additional manifest files also
asserted.

**Now**: harness MUST assert the following **four files** exist (chart-referenced
...
##### FR-B87-072 — MODIFIED (ADR-B87-001, Q-001 Resolution)

**Previously**: harness MUST assert `kustomization.yaml.tmpl` contains a
`resources:` block and that every `.yaml.tmpl` manifest in the subtree appears
in the `resources:` list.

**Now**: harness MUST assert that `values-forge.yaml.tmpl` contains the key
sentinel **`masterkeySecretName`** AND the Aegis annotation sentinel
...
with a values-overlay key-presence check). This assertion is subsumed by T-005
in the testing strategy (ADR-B87-001, `design.md` Testing Strategy).

##### BDD Scenario 1 Then-clause — MODIFIED (ADR-B87-001, Q-001 Resolution)

**Previously** (specs.md BDD, Scenario 1, Then):
```gherkin
Then the 2.0.0/infra/zitadel/ subtree exists with kustomization.yaml.tmpl,
  README.md.tmpl, docker-compose.fragment.yml.tmpl, and all manifest templates
And kustomization.yaml.tmpl lists every manifest template file as a resources: entry
```
...
All other Then-clauses in Scenario 1 remain in force.

---

## Anti-Hallucination Pass (Article III.4)

- **Introduction, not migration (central finding)** — B8-BASELINE §7 records
  the delta as `| implicit auth | Zitadel | B.8.7 |`; the 1.0.0 stack has NO
  deployed identity component. "Migration" language in plan §4.2 is misleading
  for the flagship — this is an additive introduction. RECORDED in Ground-Truth
  section; every FR is scoped accordingly.
- **All Zitadel-specific version pins deferred to verify-then-pin LIVE** — no
  Helm chart version, no container image tag, no registry identity (docker.io vs
  ghcr), no Zitadel crate/package version is asserted as registry-verified in
  this spec. Q-001 and ADR-B87-001 gate these at `/forge:design` (b8-coroot
  lesson: registries rot; hosts move).
- **Bootstrap env-var names NOT fabricated** — the spec intentionally does NOT
  write `ZITADEL_FIRSTINSTANCE_ORG_NAME` or any other `ZITADEL_FIRSTINSTANCE_*`
  variable name as a fact. These are referenced as "per live Zitadel documentation
  resolved at /forge:design (Q-003/ADR-B87-003)." Any concrete env-var name in
  the spec before a live-doc check is an Article III.4 violation.
- **Masterkey constraints not asserted** — minimum masterkey length, generation
  command, and storage requirements are sourced from live Zitadel docs at design
  (FR-B87-023). Not guessed here.
- **Postgres compatibility not asserted** — whether Postgres 17 is in Zitadel's
  supported version matrix is verified live at design (FR-B87-033 / Q-002 /
  ADR-B87-002). Not assumed.
- **Login-UI topology not assumed** — whether the current Zitadel line splits
  the login UI into a separate container (v3 login-ui) or ships as a monolith is
  Q-004, resolved at `/forge:design` (ADR-B87-001/003). This affects the
  kustomization resource list and the compose fragment service count. NOT
  fabricated here.
- **Envoy OIDC wiring excluded** — no SecurityPolicy, JWTAuthn filter, or
  backend auth middleware is specced or implied. FR-B87-063/064 document the
  delegation contract; all wiring is deferred (ADR-B87-006).
- **2.0.0.yaml delivered-flip annotation shape** — the exact comment format is
  resolved at `/forge:design` by inspecting the live B.8.4/B.8.6 annotations in
  2.0.0.yaml (FR-B87-053). Not fabricated here.
- **b8-3 test coupling** — re-read of 2.0.0.yaml confirms the `zitadel`
  component uses `name`, `standard:`, `delivered_by`, `replaces`, `role` fields —
  none in the forbidden set `{version, pin, image}` (b8-3 T-012). A
  `# B.8.7 — delivered` YAML comment is not a scalar value — T-015 does not flag it.
  FR-B87-051 is grounded in direct re-read of the b8-3 test assertions.
- **Independent review required (NFR-B87-009)** — these specs MUST pass an
  independent reviewer before `/forge:design`. Not self-approved here.

## Open Questions

Tracked in `open-questions.md`: Q-001 (install source + image/chart pins — live
registry verify at /forge:design → ADR-B87-001, open), Q-002 (datastore topology —
shared fsm-db vs dedicated → ADR-B87-002, open), Q-003 (bootstrap mechanism —
FIRSTINSTANCE env vs Admin-API Job vs CLI; masterkey; signing-key rotation →
ADR-B87-003, open), Q-004 (login-UI topology — separate container vs monolith;
kustomization resource list → ADR-B87-001/003, open), Q-005 (Envoy OIDC
delegation scope — document-only here, minimal SecurityPolicy example at design? →
ADR-B87-006, open, lean document-only).
