...
- [x] **T-PHA-001** : Create `.forge/scripts/tests/j7.test.sh` with
...
- [x] **T-PHA-002** : Add **16 L1 test stubs** (`_test_j7_001` →
...
- [x] **T-PHA-006** : Add **NFR-J7-001 perf guard** as
...
- [x] **T-PHA-009** : RED gate confirmed —
...
- [x] **T-SCH-007** [P] : Add `properties.enforcement` object with
      `required: [ci_blocking, pre_commit_hook]`,
      sub-properties booleans, `additionalProperties: false`.
      [Story: FR-J7-008]
- [x] **T-SCH-008** [P] : Add `properties.forbidden`
      (`type: array`, `items: {type: string}`, allow empty `[]`).
      [Story: FR-J7-009]
- [x] **T-SCH-009** [P] : Add `properties.rationale`
      (`type: string`, `minLength: 1`). [Story: FR-J7-010]
- [x] **T-SCH-010** : Set root-level `additionalProperties: true`
...
- [x] **T-VAL-003** : Embed the Python 3 inline engine via
...
- [x] **T-VAL-004** : Implement Phase 1 schema validation : walk
...
- [x] **T-VAL-005** : Implement the deterministic error format —
...
- [x] **T-VAL-006** : Run `j7.test.sh` ; assert L1 tests
...
- [x] **T-VAL-007** : `j7.test.sh` `Passed: ≥ 10 / Failed: ≤ 7`,
...
- [x] **T-INV-001** : RED witness — convert
      `_test_j7_020_xii_coupling` ; capture log. [Story: FR-J7-020]
- [x] **T-INV-002** : Implement bidirectional Article XII coupling :
...
- [x] **T-INV-003** [P] : Implement `expires_at > last_reviewed`
      strict ordering when both are dated. Use `datetime.date`
      comparison (post-coercion). [Story: FR-J7-021]
- [x] **T-INV-004** [P] : Implement non-blocking 12-month cycle
...
- [x] **T-INV-005** : Implement REVIEW.md drift check (full ledger
...
- [x] **T-XREF-003** [P] : RED + impl
      ...
      [Story: FR-J7-050]
- [x] **T-XREF-004** [P] : Implement reverse coverage as
...
- [x] **T-FBD-001** [P] : RED + impl `_test_j7_041_forbidden_dup` —
...
- [x] **T-VFY-001** : RED witness — extend
...
- [x] **T-VFY-002** : Edit `.forge/scripts/verify.sh` — add new
...
- [x] **T-VFY-003** : Honor `examples/` skip-guard convention from
      F.2 — any `examples/**/.forge/standards/*.yaml` is skipped
      without count. [Story: FR-J7-072]
- [x] **T-VFY-004** : Run `verify.sh` end-to-end against the live
...
- [x] **T-L2-001** : Implement `_test_j7_l2_good_fixture` — temp dir
...
- [x] **T-L2-002** : Implement `_test_j7_l2_bad_fixture_six_modes` —
...
- [x] **T-L2-003** : Implement `_test_j7_l2_drift_fixture` —
...
- [x] **T-L2-004** : Implement `_test_j7_l2_perf_budget` — wrap
      live-tree run in `time` (or Python `time.perf_counter`) ;
      assert ≤ 2 s. [Story: NFR-J7-001]

### Phase 3 gate

- [x] **T-INT-001** : Run `j7.test.sh --level 1,2` end-to-end ;
...
- [x] **T-STD-001** [P] : Add a section "Automated enforcement" to
...
- [x] **T-DOC-001** [P] : Add a new section "Standard YAML schema"
      ...
      [Story: FR-J7-101]
- [x] **T-DOC-002** [P] : Add an entry to `CHANGELOG.md` under
...
- [x] **T-CI-001** : Confirm `j7.test.sh` is in the `forge-ci.yml`
...
- [x] **T-REV-001** : Run `/forge:review j7-validate-standards-yaml`
...
- [x] **T-REV-003** : Verify `bin/forge-questions.sh
