...
- [ ] **T1.1** Add `_test_b810_l1_013_migration_output_rendered` to
      ...
      [Story: FR-B810B-005]
- [ ] **T1.2** Run it against **today's** `cp`-based script. **Confirm RED**, and
...
- [ ] **T2.1** Author
      ...
      [Story: FR-B810B-001]
- [ ] **T2.2** Assert the plan covers the tree exactly: entry count == file count
...
- [ ] **T3.1** In `forge-migrate-flagship.sh`: read the three substitution values
      from the target manifest; exit **7** naming the key if any is missing.
      [Story: FR-B810B-003]
- [ ] **T3.2** Render the 2.0.0 set via `overlay.sh --plan migration-plan-2.0.0.yaml`
...
- [ ] **T3.3** Retire `_b810_map_relpath` — the plan's `target:` does the mapping.
      [Story: FR-B810B-001]
- [ ] **T3.4** Run the harness. **Confirm GREEN**, including the nine previously
      shadowed files now going through `_a7_classify`.
      [Story: FR-B810B-004]

## T4 — idempotence, the requirement most at risk

- [ ] **T4.1** Second-run assertion: migrate twice into the same target; the second
      run must report zero `upgraded` and leave the tree byte-identical.
      [Story: NFR-B810B-003]
- [ ] **T4.2** Probe the exclusion specifically: with the temp manifest NOT excluded,
      the second run must fail the assertion. If it passes either way, the exclusion
      is not what makes idempotence hold and the design note is wrong.
      [Story: NFR-B810B-003]

## T5 — prove the tests are not vacuous

- [ ] **T5.1** Mutation-probe each assertion of T1.1 independently: reintroduce a
      raw `cp` for one file; leave one placeholder unsubstituted; recreate one
      shadowing `X.tmpl`. Each must red its own assertion.
      [Story: FR-B810B-005]
- [ ] **T5.2** Probe the manifest-refusal path: remove `project_name` from the
...
- [ ] **T6.1** L2 opt-in test: render a real 1.0.0 project, migrate, assert on the
      actual 36-file result. Gated on the toolchain, skip-when-absent.
      [Story: FR-B810B-005]
- [ ] **T6.2** Execute it once locally and record the counts in `evidence.md` —
      before/after, so the 36 / 24 / 9 figures have an after column.
      [Story: FR-B810B-005]

## T7 — docs + banner

- [ ] **T7.1** Fix the stale banner at `:175` (`scaffoldable: false until B.8.14`).
      [Story: FR-B810B-006]
- [ ] **T7.2** `docs/MIGRATIONS.md`: describe what is actually delivered, and add
      the already-migrated-adopter cleanup note (Q-004) — which files are strays and
      that the tool will not delete them.
      [Story: FR-B810B-007]
- [ ] **T7.3** `CHANGELOG.md` `[Unreleased]`, calling out the behavioural change:
      nine files that previously landed beside their rendered twin now merge.
      [Story: FR-B810B-007]

## T8 — regression

- [ ] **T8.1** `b8-10.test.sh` full; `b8-2.test.sh` (frozen 1.0.0 snapshot must not
      drift); `b9-2.test.sh::T-010` (`overlay.sh` byte-unchanged — this change must
      not touch it).
      [Story: NFR-B810B-004]
- [ ] **T8.2** Full 80-entry CI matrix sweep (match the bare-invocation entries too,
      not only `--level` ones) + shellcheck.
      [Story: NFR-B810B-004]
- [ ] **T8.3** `verify.sh` + `constitution-linter.sh` **after** flipping
      `.forge.yaml` to `implemented`.
      [Story: NFR-B810B-004]

## Negative scope guard

- [ ] **T9.1** `git diff --stat`: no change to `overlay.sh`, the `_a7_*` library in
