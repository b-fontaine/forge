...
- Every `uses:` reference pinned to a tag (no `@main` / `@master`
  / `@HEAD` / `:latest`).
- Workflow file ≤ 420 lines (NFR-CI-002 ; bumped 250→300 on 2026-05-12, 300→340 on 2026-06-23 for b7-7-example's second example-tree RAG gate, 340→380 on 2026-06-23 for b7-6-harness's `harness-rust` live codegen/build job, 380→400 on 2026-07-12 for b6-7-harness's event-driven-eu promotion gate `harness-rust` L2 step, then 400→420 on 2026-07-12 for b6-8-example's third example-tree EDA gate + the `b6-8.test.sh` harness-loop entry, then 420→440 on 2026-09-13 for B.3 (`t7-ci-line-budget-440`), which cannot register `b3-1.test.sh` at 420/420 — B.9.11 had spent the last reserved line. The growth is linear in harness entries: the array is 102 of the file's 420 lines and costs one line per harness.
- The budget is asserted in **five** harnesses — c1, g1, t5-1, t5-otel-live-run **and b6-8** — bump them in lock-step. This list said *four* until 2026-09-13; `b6-8.test.sh:73` had added a `CI_LINE_BUDGET` of its own without updating it. The set here was re-measured empirically, by pushing the workflow one line over the cap and recording which harnesses fired, rather than trusted.
- **The per-harness cost is unresolved.** Externalising the array into a file the workflow reads would free ~80 lines and end the growth, but 30 harnesses read `forge-ci.yml` and at least seven grep it for their own registration (b4, b6-8, b7-7, f1, f2, f4, d5), so it is a brick of its own. Revisit at 440.
