# Spec: b8-schema-candidate

<!-- Audit: B.8.3 (b8-3-schema-candidate) -->
<!-- Source change : `.forge/changes/b8-3-schema-candidate/` (delta specs.md authoritative). -->
...
This spec is the 2.0.0 candidate target schema for the `full-stack-monorepo`
archetype. It gates all downstream B.8.4–B.8.12 bricks: each downstream
template-authoring step builds **against** the component set, layer topology,
and breaking-deltas declared here. The candidate schema is delivered as
`.forge/schemas/full-stack-monorepo/2.0.0.yaml` — intentionally invisible to
existing validators (which read `schema.yaml` by literal filename) and validated
by its own `b8-3.test.sh` harness; the shared-validator rewiring is B.8.3.b.
The 1.0.0 `schema.yaml` surface remains maintenance-frozen (B.8.2) and
byte-unchanged; both files coexist until B.8.14 promotes 2.0.0 to `stable`.
This spec is reference-only and frozen-1.0.0-safe.
