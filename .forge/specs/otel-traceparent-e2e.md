# Spec: otel-traceparent-e2e

<!-- Audit: T.5 Phase C (t5-otel-traceparent-e2e) — W3C traceparent end-to-end validation through Kong gateway. -->
<!-- Source change : `.forge/changes/t5-otel-traceparent-e2e/` (archived 2026-05-26). -->
<!-- Phase A counterpart : `.forge/specs/otel-stack.md` (infra side, archived 2026-05-10). -->
<!-- Phase B counterpart : `.forge/specs/otel-app.md` (SDK side, archived 2026-05-12). -->
...
Inline `[NEEDS CLARIFICATION:]` markers : none in this spec.
All ambiguities resolved at `/forge:design` time :

- **Q-001** (FR-T5-TPE-006, sampled-off path SDK behaviour) →
  resolved in `design.md` ADR-T5-TPE-001 via Context7 review of
  `/open-telemetry/opentelemetry-rust` ParentBased sampler.
- **Q-002** (Envoy forward-pointer change name) → resolved in
  `design.md` § Out of scope ; named change is `b8-envoy-migration`.
...
- **Phase A (`t5-otel-stack`, archived 2026-05-10)** : infra-side
  templates.
- **Phase B (`t5-otel-app`, archived 2026-05-12)** : app-side SDK
  instrumentation in the flagship example.
- **Phase C (this change, archived 2026-05-26)** : BDD scenario
  text + Kong preservation contract + harness gate for the
  traceparent E2E loop.
- **Phase D (deferred)** : Flutter `bdd_widget_test` + Rust
  `cucumber-rs` step bodies + docker-compose live-run driver +
  Envoy migration validation (`b8-envoy-migration`).
