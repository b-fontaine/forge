# Spec: b8-e2e-migration

<!-- Audit: B.8.12 (b8-12-e2e-migration) -->
<!-- Source change : `.forge/changes/b8-12-e2e-migration/` (delta specs.md authoritative). -->

**Namespace** : `FR-B812-*` / `NFR-B812-*` / `ADR-B812-*`.
...
B.8.12 delivers the **E2E migration convergence gate**: a golden span-inventory
regression asserting the 2.0.0 after-state is a superset-OR-EQUAL of the 1.0.0
baseline (3 code-verified spans — NO committed p95/p99, latency is
methodology-only per ADR-B8-1-002 / III.4) plus a hermetic migration driver
invoking `forge-migrate-flagship.sh` against a tmpdir copy of c1 (committed c1
stays 1.0.0; dry-run L1 + opt-in real overlay L2). The change also lands the
two named deferrals:

- **Rust S2S Connect client** (`transport_connect_client.rs.tmpl`,
  connectrpc::client 0.6.x — `ClientConfig::new` takes `http::Uri`, caught by
  L2 cargo-check; ADR-B86-004 target landing).
- **Envoy-OIDC wiring** (SecurityPolicy `gateway.envoyproxy.io/v1alpha1`
  `spec.jwt.providers[].remoteJWKS` → Backend, issuer = ExternalDomain,
  jwks `<issuer>/oauth/v2/keys`; backend JWT middleware via jwt-authorizer
  0.15.x; ADR-B87-006 target landing).

0-regression asserted on demo-001..004 (Spec Delta: 3 `.feature` files;
demo-004 spec-frozen). `b8-12.test.sh` 23 L1 + 4 L2 opt-in; full suite 51/51;
pure additive, no standard bump; independent review design (Spec Delta round) +
final APPROVE; archived 2026-06-04.
...
- **Rust S2S Connect client is a named deferral landing here.** ADR-B86-004
  ...
  (`ClientConfig`, `CallOptions`, etc.) were verify-then-pin LIVE at
  /forge:design (ADR-B812-003). Transport.yaml `versions_2_0_0` block pins
  (`connectrpc =0.6.1`, `buffa =0.6.0`) are the AUTHORITATIVE observed pins
  (transport.yaml v1.3.0, observed 2026-06-04). **Resolved (ADR-B812-003, live
  2026-06-04): `connectrpc::client::{HttpClient, ClientConfig, CallOptions}`;
  `ClientConfig::new(uri: http::Uri)` (NOT `&str` — caught by L2 cargo-check);
  `client`/`client-tls` Cargo features.**
- **Envoy-OIDC wiring is a named deferral landing here.** ADR-B87-006
  ...
  B.8.12. **Resolved (ADR-B812-002, live 2026-06-04): `apiVersion:
  gateway.envoyproxy.io/v1alpha1`, `kind: SecurityPolicy`, JWT folded into
  `spec.jwt.providers[]` (Envoy Gateway v1.8.0; no separate JWTAuthn resource);
  issuer `https://<ExternalDomain>`; jwks_uri `<issuer>/oauth/v2/keys`
  (confirmed via live Zitadel discovery + zitadel source
  `op.NewEndpoint("/oauth/v2/keys")`).** Identity.yaml v1.1.0 records the
  Zitadel chart-tested pair: chart `10.0.2` / appVersion `v4.14.0` (ghcr.io,
  v-prefix convention).
...
- **"4 demos" = demo-001..004 (demo-005 is T.5, separate).** The 0-regression
  assertion covers demo-001..004. Qwik→OTel is NOT B.8.12 — routes to B.7
  (ai-native-rag).
- **Spec Delta (ADR-B812-004, design-phase).** Design-phase live evidence falsified
  the demo `.feature` path assumptions in Group 3. The live layout: demo-001/002/003
  each ship a feature file at
  `examples/forge-fsm-example/.forge/changes/demo-00N-*/features/*.feature`
  (NOT `test/features/`); **demo-004 has NO feature file** (`status: specified`,
  spec-frozen). FRs 020/022/023 corrected per the Spec Delta (see below).
- **migrate-against-tmpdir-copy (committed c1 stays 1.0.0).** The harness
  drives `forge-migrate-flagship.sh --target <tmpdir>` against a fresh `cp -r`
  of the c1 example; the committed `examples/forge-fsm-example/` is git-clean
  after every test. FR-B812-012 and NFR-B812-010 assert this machine-verifiably.
...
| **ADR-B812-002 (as-implemented)** | SecurityPolicy `gateway.envoyproxy.io/v1alpha1`, JWT folded into `spec.jwt.providers[].remoteJWKS`→Backend, issuer `https://<ExternalDomain>`, jwks `<issuer>/oauth/v2/keys`; backend JWT middleware jwt-authorizer 0.15.x |
| **ADR-B812-003 (as-implemented)** | `connectrpc::client::{HttpClient, ClientConfig, CallOptions}`; `ClientConfig::new(uri: http::Uri)` (NOT `&str`); `client`/`client-tls` features; caught by L2 cargo-check |
| **ADR-B812-004 (as-implemented)** | Spec Delta: demo `.feature` paths corrected (live layout `.forge/changes/demo-00N-*/features/`); demo-004 has no feature file; FRs 020/022/023 corrected at design |
...
##### FR-B812-020 — demo-001..003 `.feature` files byte-intact after additive overlay
<!-- Modified at design-phase by ADR-B812-004 (demo .feature paths), 2026-06-04 -->
The **three** demo `.feature` files at
...
<!-- Modified at design-phase by ADR-B812-004 (demo .feature paths), 2026-06-04 -->
The demo-004 entry at
`examples/forge-fsm-example/.forge/changes/demo-004-user-onboarding/.forge.yaml`
MUST retain `status: specified` after the migration overlay (the overlay is additive,
...
<!-- Modified at design-phase by ADR-B812-004 (demo .feature paths), 2026-06-04 -->
The harness MUST include a structural check: for each of the **3** demo `.feature`
files (demo-001/002/003), confirm the `Feature:` + at least one `Scenario:` +
Given/When/Then structure is intact (mirroring the `_test_olr_010_feature_file_exists`
awk pattern). **demo-004 (no feature file) is excluded.** This is a sentinel that
the overlay did not truncate or corrupt any `.feature` file. Testable: the harness
awk-based Given/When/Then check exits 0 for all **3** demo feature files.
...
client API symbols (`ClientConfig::new` takes `http::Uri` — NOT `&str`, caught by
L2 cargo-check, per ADR-B812-003) are verified at implement. Testable:
`grep "=0.6" .forge/templates/archetypes/full-stack-monorepo/2.0.0/backend/crates/grpc-api/src/transport_connect_client.rs.tmpl || grep "=0.6" <companion-Cargo.toml.tmpl>` → exit 0.
...
(context deadline forwarded to outbound calls); (d) retry policy posture.
The minimum testable requirement: `grep -iE "auth|tls|deadline|retry" <template-path>` → exit 0.
...
At minimum a SecurityPolicy template and a backend JWT validation middleware template
MUST be present. Testable: `find .forge/templates/archetypes/full-stack-monorepo/2.0.0/infra/k8s/envoy-gateway/ -name "*security*" -o -name "*jwt*" -o -name "*oidc*"` → at least one file found.

##### FR-B812-041 — Envoy Gateway SecurityPolicy apiVersion/kind verify-then-pin LIVE (resolved)
**Resolved (ADR-B812-002, live 2026-06-04):** `apiVersion: gateway.envoyproxy.io/v1alpha1`,
`kind: SecurityPolicy`, JWT folded into `spec.jwt.providers[]` (Envoy Gateway v1.8.0;
no separate JWTAuthn resource). Testable at implement:
`grep "apiVersion" <security-policy-template>` must match `gateway.envoyproxy.io/v1alpha1`.

##### FR-B812-042 — Backend JWT validation middleware template present
A backend JWT validation middleware template MUST be present under
`.forge/templates/archetypes/full-stack-monorepo/2.0.0/backend/` (jwt-authorizer
0.15.x middleware, per ADR-B812-002). Both gateway-side and backend middleware
land in B.8.12. Testable:
`find .forge/templates/archetypes/full-stack-monorepo/2.0.0/backend/ -name "*jwt*" -o -name "*auth*middleware*"` → at least one file found.

##### FR-B812-043 — Zitadel OIDC discovery URL pattern verify-then-pin LIVE (resolved)
**Resolved (ADR-B812-002, live 2026-06-04):** issuer `https://<ExternalDomain>`;
jwks_uri `<issuer>/oauth/v2/keys` (confirmed via live Zitadel discovery +
zitadel source `op.NewEndpoint("/oauth/v2/keys")`). Testable at implement: the
Envoy-OIDC template references the verified Zitadel OIDC discovery URL pattern.
...
registry, v-prefix convention). Testable:
`grep "identity.yaml" <envoy-oidc-template>` → exit 0 (or `grep "1.1.0"` in an audit comment within the template).
...
##### FR-B812-050 — Methodology doc present (home: `docs/MIGRATIONS.md` extension)
...
(p99 >20%, traceparent errors >1%). Per ADR-B812-001: the home is an extension
to `docs/MIGRATIONS.md` (a new `§B.8.12` section). The doc MUST reference
`docs/B8-BASELINE.md §6` (the re-measurement methodology) and the B.8.13 rollback
criteria. Testable: `[ -f docs/MIGRATIONS.md ]` → exit 0; `grep "p99" docs/MIGRATIONS.md` → exit 0;
`grep "B.8.13" docs/MIGRATIONS.md` → exit 0.
...
live numbers. Testable: `grep -E 'p9[59].*[0-9]+(ms|µs|s\b)' docs/MIGRATIONS.md` → zero matches.
...
##### FR-B812-062 — L1 hermetic assertions (23 tests, ≤ a few seconds wall-clock)
**Delivered: 23 L1 tests, all GREEN.** The L1 block asserts all of the following
...
9. demo-001..003 feature file Given/When/Then structure intact (FR-B812-023, 3 files).
...
19. Fake-OTLP collector stdlib-only guard (FR-B812-063).
20. methodology doc present + p99 methodology grep (FR-B812-050).
21. 1.0.0 server adapter byte-unchanged (FR-B812-033).
22. 1.0.0 Kong infra paths byte-unchanged (FR-B812-045).
23. Envoy-OIDC apiVersion sentinel in SecurityPolicy template (FR-B812-041).
...
(d) cargo-check on rendered S2S client template when `FORGE_E2E_TOOLCHAINS=1`
(FR-B812-034/065). When `FORGE_B8_12_LIVE` is unset, ALL L2 tests MUST emit
skip-pass (0 failures). **Delivered: 4 L2 opt-in tests.**
...
not a cargo build). **DELIVERED: 23 L1 tests, all GREEN.**
...
At `/forge:design`, the implementer verified LIVE and pinned in ADR-B812-002 and
ADR-B812-003: (a) the exact connectrpc 0.6.x client API symbols used in
`transport_connect_client.rs.tmpl`; (b) the Envoy Gateway SecurityPolicy
`apiVersion`/`kind` and JWT filter shape; (c) the Zitadel OIDC discovery URL pattern.
**RESOLVED at implement (live 2026-06-04): (a) connectrpc::client {HttpClient,
ClientConfig, CallOptions} + `client`/`client-tls` features (ClientConfig::new
takes `http::Uri`, caught by L2 cargo-check); (b) Envoy SecurityPolicy
gateway.envoyproxy.io/v1alpha1 spec.jwt.providers[]; (c) Zitadel discovery issuer
...
These specs passed an independent reviewer (not the author) before `/forge:design`
proceeded (t5-2 self-validation lesson; b8-coroot inversion lessons). Independent
review completed pre-archive at both design (Spec Delta round) and final.
**DELIVERED: independent reviewer issued final APPROVE.**
...
|-----|----------|--------------------------|
| **ADR-B812-001** | before/after mechanism + methodology doc home (Q-001 + Q-005) | **RESOLVED: golden span-inventory superset (hermetic, no committed latency); methodology doc = `docs/MIGRATIONS.md §B.8.12` extension.** The 2.0.0 after-state golden is a superset-OR-EQUAL of the 1.0.0 baseline 3 code-verified spans. No committed p95/p99 number anywhere (ADR-B8-1-002 + III.4 binding). Methodology doc in MIGRATIONS.md describes the re-measurement procedure and B.8.13 rollback thresholds (relative percentages only). |
| **ADR-B812-002** | Envoy-OIDC scope + API verify-then-pin (Q-002) | **RESOLVED (live 2026-06-04): Envoy Gateway v1.8.0 folds JWT into `SecurityPolicy.spec.jwt.providers[].remoteJWKS.backendRefs`→Backend — no separate JWTAuthn resource.** `apiVersion: gateway.envoyproxy.io/v1alpha1`, `kind: SecurityPolicy`. Zitadel issuer `https://<ExternalDomain>`; jwks_uri `<issuer>/oauth/v2/keys` (confirmed via live Zitadel discovery + source `op.NewEndpoint("/oauth/v2/keys")`). Both gateway-side SecurityPolicy and backend jwt-authorizer 0.15.x middleware land in B.8.12. |
| **ADR-B812-003** | S2S client surface + auth/TLS/deadline/retry posture (Q-003) | **RESOLVED (live 2026-06-04): `connectrpc::client::{HttpClient, ClientConfig, CallOptions}`; `ClientConfig::new(uri: http::Uri)` (NOT `&str` — caught by L2 cargo-check); `client`/`client-tls` Cargo features.** Minimal client on connectrpc 0.6.x with documented auth/TLS/deadline/retry posture via inline comments; cargo-check gated behind L2 opt-in `FORGE_E2E_TOOLCHAINS`. |
| **ADR-B812-004** | migration target mechanics + demo `.feature` path correction (Q-004 + Spec Delta) | **RESOLVED: tmpdir copy of `examples/forge-fsm-example/` + `forge-migrate-flagship.sh --target`; committed c1 stays 1.0.0; real overlay L2 opt-in; dry-run plan assertion L1. Spec Delta (design-phase 2026-06-04): demo `.feature` paths corrected — live layout is `examples/forge-fsm-example/.forge/changes/demo-00N-*/features/*.feature`; demo-004 has NO feature file (spec-frozen). FRs 020/022/023 corrected in this spec.** |
