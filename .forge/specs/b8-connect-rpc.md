...
<!-- Source change : `.forge/changes/b8-6-connect-rpc/` (delta specs.md authoritative). -->
...
see Ground-Truth section below), IV (delta-based: all deliverables are additive;
1.0.0 frozen assets untouched), V (harness + gates must pass before status flips),
VII.2 (tonic gRPC SHALL — in force, PRESERVED), VIII.1 (Kong SHALL — in force,
PRESERVED).

## Overview

B.8.6 delivers the **Connect-RPC 2.0.0 transport brick**: a versioned subtree
`2.0.0/shared/protos/` (buf.gen.yaml.tmpl + README.md.tmpl) consecrating
Connect-RPC as the 2.0.0 default transport, Rust crate-pin modernization
`connectrpc`/`connectrpc-build` `=0.6.1` + `buffa`/`buffa-types` `=0.6.0`
(verify-then-pin LIVE 2026-06-02T13:35Z — crates.io evidence.md P-12),
`transport.yaml` v1.2.0 → v1.3.0 additive (`codegen.versions_2_0_0:` sibling
block), `2.0.0.yaml` `connect-rpc` component `# B.8.6 — delivered` annotation,
`b8-6.test.sh` 12 L1 hermetic, forge-ci.yml registration, CHANGELOG entry, and
S2S Rust client re-deferred to B.8.12 (ADR-B86-004). connect-go v1.19.2 →
v1.20.0 (BSR-confirmed at implement). Sibling harnesses t5 / t5-cargo version-pin
windows bumped. Independent review APPROVE. Archived 2026-06-02.
...
shipped official identity `buf.build/connectrpc/dart` (Q-004 — verified live at
implement: `v1.0.0` still current). The real B.8.6 delta is:
...
2. **Rust Connect crate-pin modernization** for the 2.0.0 line only:
   `connectrpc`/`connectrpc-build` `=0.6.1` + `buffa`/`buffa-types` `=0.6.0`
   (verify-then-pin LIVE crates.io 2026-06-02T13:35Z; `ConnectRouter` +
   `into_axum_service()` API surface confirmed unchanged on 0.6.x).
3. **`transport.yaml` additive bump v1.2.0 → v1.3.0**: `codegen.versions_2_0_0:`
   sibling block for 2.0.0 pins + stale "v1.1.0" header comment fixed + REVIEW.md
   ledger entry.
...
6. **Harness `b8-6.test.sh`** (12 L1) + `forge-ci.yml` registration + CHANGELOG.
7. **connect-go v1.19.2 → v1.20.0** (BSR-confirmed at implement).
8. **S2S Rust Connect client re-deferred to B.8.12** (ADR-B86-004).

## Source Documents

| Field | Value |
|-------|-------|
| **Plan ref** | `docs/new-archetypes-plan.md` §4 (Module B.8), §4.2 B.8.6 (GROUND-TRUTH NOTE: plugin extension partially stale; see above) |
...
| **2.0.0 template tree (observed)** | `.forge/templates/archetypes/full-stack-monorepo/2.0.0/` contains `infra/k8s/envoy-gateway/` (B.8.4) and `infra/postgres/` (B.8.5). No `shared/protos/` subtree existed before this brick. |
| **ADR-T5-003 (observed)** | Demo-005 ships TS-only; Rust S2S Connect client deferred to B.8 (T6). Re-deferred to B.8.12 by this brick (ADR-B86-004). |
| **Standard bump precedent** | `transport.yaml` 1.0.0 → 1.1.0 → 1.2.0 additive; `gateway.yaml` created by B.8.4 (REVIEW.md precedent). REVIEW.md is append-only (Article XII). J.7 = `bin/validate-standards-yaml.sh`. |
...
| **Dependencies** | B.8.3, B.8.4, B.8.5, T.5 (`t5-connect-codegen` + `t5-cargo-pin-refresh`). |
| **Release target** | v0.4.0-rc.9 |
...
if `v1.0.0` is still current, it is retained. **RESOLVED at implement: `v1.0.0`
still current (BSR-confirmed).** The 1.0.0 frozen manifest stays
`buf.build/connectrpc/dart:v1.0.0` byte-for-byte.
...
propose/specify). **RESOLVED at /forge:design via live crates.io evidence
...
pins block** added by FR-B86-021 (`codegen.versions_2_0_0:`) carries the
modernized versions.
...
because 0.3.x stopped at 0.3.0). **RESOLVED: `buffa`/`buffa-types` `=0.6.0`
compat confirmed live at /forge:design.**

##### FR-B86-013 — axum integration shape verified on the modern line
The `Router::into_axum_service()` / `ConnectRouter` integration shape
(FR-T5-CC-010) MUST be verified against the chosen modern `connectrpc` crate at
`/forge:design` before the line is selected. **RESOLVED at implement: API surface
(`ConnectRouter` + `into_axum_service()`) unchanged on the 0.6.x line; adapter
rewritten on the 0.6.x surface.**

##### FR-B86-014 — Rust S2S Connect client: land or explicitly re-defer
ADR-T5-003 deferred the Rust S2S Connect client to B.8 (T6). This brick has
explicitly **re-deferred it to B.8.12** with a dated note in `design.md` and an
updated reference in `.forge/specs/connect-codegen.md` ADR-T5-003 (→ ADR-B86-004,
Q-003). A silent omission (neither landed nor re-deferred) is a constitutional
violation — this requirement is satisfied by the explicit re-deferral record.
...
`transport.yaml` under the `codegen.versions_2_0_0:` block added by FR-B86-021.
...
The v1.3.0 transport.yaml MUST add a `codegen.versions_2_0_0:` sibling block
(ADR-B86-005 RESOLVED: `codegen.versions_2_0_0:` key chosen at /forge:design)
containing the modernized 2.0.0-line Rust crate pins (`connectrpc`, `connectrpc-build`,
`buffa`, `buffa-types`) for `schema: "2.0.0"` only. The 1.0.0 and 2.0.0 pin sets
are separately addressable by schema version so tooling and harnesses can
distinguish them.

##### FR-B86-022 — stale "v1.1.0" header comment fixed
The `transport.yaml` top-of-file audit-comment block still said "v1.1.0" (drift
...
`transport.yaml v1.3.0`, dated 2026-06-02 (the B.8.6 implement date), with a
one-line description of the change (additive: 2.0.0-line crate pins + header
fix). Mirrors the B.8.4 `gateway.yaml` and B.8.5 `orchestration.yaml` REVIEW.md
...
through the v1.3.0 bump (FR-B86-011). Only the `codegen.versions_2_0_0:` block
and the header comment are new or changed.

##### FR-B86-026 — No WAIVER required for the transport.yaml bump
The v1.2.0 → v1.3.0 bump is purely additive (new `codegen.versions_2_0_0:` block
...
consistently annotated across delivered bricks. The formatting (spacing,
em-dash) mirrors the live B.8.4 annotation (inspected at /forge:design).
...
`buf.build/connectrpc/dart` — Article III.4). **RESOLVED at implement: official
plugin `buf.build/connectrpc/dart:v1.0.0` confirmed as current (BSR-verified).**
...
`b8-5.test.sh` line. **DELIVERED: 12 L1 tests, all GREEN.**
...
The harness MUST assert the v1.3.0 transport.yaml contains the `codegen.versions_2_0_0:`
block with at least the four Rust crate keys (`connectrpc`, `connectrpc-build`,
`buffa`, `buffa-types`) referencing the modernized 0.6.x versions for the 2.0.0
line (FR-B86-021 / ADR-B86-005 RESOLVED).
...
`# B.8.6 — delivered` annotation comment and that `standard: transport.yaml` still
resolves (FR-B86-030 / FR-B86-031). The `rest-bridge → connect-rpc` migration_delta
MUST still be present with `strategy: additive-first` (FR-B86-032).
...
modernization, transport.yaml v1.3.0, 2.0.0.yaml annotation, harness). **DELIVERED.**
...
`2.0.0/shared/protos/` subtree. **DELIVERED: full suite 45/45 GREEN.**
...
check). **DELIVERED: pins resolved live 2026-06-02T13:35Z — `=0.6.1`/`=0.6.0`.**
...
`/forge:design` proceeds. Self-approval of the anti-hallucination pass
(Article III.4) is prohibited. **DELIVERED: independent review APPROVE recorded.**
...
|-----|----------|--------------------------|
| **ADR-B86-001** | connectrpc crate target line for 2.0.0 | `connectrpc`/`connectrpc-build` `=0.6.1` + `buffa`/`buffa-types` `=0.6.0`; lockstep verify-then-pin LIVE crates.io 2026-06-02T13:35Z; buffa compat matrix resolves; `ConnectRouter` + `into_axum_service()` API surface unchanged on 0.6.x |
| **ADR-B86-002** | 2.0.0 subtree shape | Full-copy `buf.gen.yaml.tmpl` (lean confirmed: self-contained artifact); subtree option b with adapter `transport_connect.rs.tmpl` + `Cargo.toml.tmpl` variant files added; `N.N.N/` versioned-subtree convention inherited from B.8.4/B.8.5 |
| **ADR-B86-003** | Connect-Dart posture | Keep `buf.build/connectrpc/dart` as 2.0.0 default; gRPC-Web-via-Envoy fallback documented; `v1.0.0` still current (BSR-confirmed at implement); plan naming drift `protoc-gen-connect-dart-community` corrected |
| **ADR-B86-004** | Rust S2S Connect client (ADR-T5-003 follow-on) | Re-deferred to B.8.12; dated note in `design.md`; updated reference in `.forge/specs/connect-codegen.md` ADR-T5-003 |
| **ADR-B86-005** | transport.yaml schema_versions block shape | `codegen.versions_2_0_0:` sibling block (additive, `breaking_change: false`, v1.3.0 not v2.0.0); shipped in transport.yaml v1.3.0 |
...
Feature: Connect-RPC consecrated as the 2.0.0 default transport via versioned subtree, standard bump, and schema annotation
  ...
  Scenario: The 2.0.0 transport subtree is created additively without disturbing the frozen 1.0.0 manifest
    ...
    And transport.yaml carries a codegen.versions_2_0_0: block with connectrpc/connectrpc-build =0.6.1 + buffa/buffa-types =0.6.0
    ...
    And b8-6.test.sh passes all 12 L1 checks within 2 s
