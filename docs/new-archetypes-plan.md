...
| 10 | `b6-8-example` — `examples/forge-eda-example/` (3 demos) | ✅ archivé 2026-07-12 (`b6-8-example`, branch `b6-8-example`) — rendu via le **vrai** `forge init --archetype event-driven-eu` (l'archétype est stable/scaffoldable depuis B.6.7), pas via `overlay.sh` (divergence de b7-7, ADR-B6-8-001) ; 3 demos archivées (ingestion HTTP→NATS, projection event-store→read-model, saga Temporal 3-steps `[backend, infra]`) ; `b6-8.test.sh` (23 checks) ; gate CI 3ᵉ arbre + budget `forge-ci.yml` 400→420 en lockstep |
