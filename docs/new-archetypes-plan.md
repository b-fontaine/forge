...
| `b8-15-upgrade-matrix` | archived | B.8.15 (T5.1 Layer D — forge-upgrade matrix test ; the v0.4.0-stable publish gate. `b8-15.test.sh` realises the N-1→N matrix as harness cells (forge-ci has no GA matrix). Direct e2e cells vs the binaries: cross-major `forge-upgrade.sh`→2.0.0 ⇒ exit 7 + `[NEEDS MIGRATION:]` ; `--force` on dirty same-major tree ⇒ refused (clean-git gate) ; migrate-flagship `--dry-run` on a c1 copy ⇒ 1.0.0→2.0.0 plan + no-op ; static additive no-`rm` guard. ADR-B815-001: positive flagship cell drives `bin/forge-migrate-flagship.sh` (runnable now) ; the `forge upgrade` front-door auto-resolve to 2.0.0 is FLIP-GATED skip-pass guard (asserts 2.0.0 still scaffoldable:false → fails-loud if the flip leaks). L2 `FORGE_B8_15_LIVE`: real overlay on c1 ⇒ upgrade_history flagship entry (1.0.0→2.0.0 + `kind`) + Kong-present additive invariant + T5.1.B fixture matrix on the overlaid tree. Same-major/ledger/.merge-conflicts gated via a7 coupling (no fixture dup) + b8-10 + b8-14. **No 2.0.0 tarball needed** (BASE=1.0.0.tar.gz, RIGHT=live 2.0.0 template-set ; plan §0.10 framing was stale). Pure test brick, no standard/schema/constitution/scaffolder/template mutation. Grounded by a 6-mapper+critic workflow ; design review CHANGES REQUIRED (1.0.0.tar.gz≠scaffolded-project + L1-dry-run-can't-assert-tree) → fixed (c1 base, L1 static guard / L2 tree) ; impl review APPROVE (re-ran L1 9/9 + L2 10/10, injection-tested the additive guard). b8-15.test.sh 9 L1 + 1 L2 ; archived 2026-06-04) |
...
| `b8-14-promotion-flip` | archived | B.8.14 **(THE FLIP — DONE 2026-06-05 ; B.8 COMPLETE 16/16)** (point-of-no-return executed in 2 ordered commits. **C1 ratification** `818ba6b` : §VIII.1 Kong→Envoy (Constitution v1.1.0→v2.0.0 MAJOR, Amendments row #2) ; BDFL ≥7-day window opened 2026-06-04 compressed to ~1 day (ratified 2026-06-05) recorded HONESTLY as a REVIEW.md Correction Entry (no fabricated completed window) ; templates→2.0.0, 46 historical changes stay 1.1.0 (d5 ADR-006) ; ARCHITECTURE-TARGET §11/§12.1 realigned Kong→Envoy / DBOS→Temporal via t4 material-path (rehash + REHASH-LOG) ; kong.md tombstone → gateway.yaml + transport.yaml ; d5 + b8-14 held-guards inverted. **C2 enablement** : 2.0.0.yaml promoted stage candidate→stable + scaffoldable false→true ; Kong-less fresh-init via new `overlay.sh`/`init.sh` `--plan` flag + `scaffold-plan-2.0.0.yaml` (1.0.0 set − Kong copy-op + 6 root/infra Kong-less `2.0.0/` variants + 7 Envoy-gateway manifests) + `bin/forge-init-fsm-2.0.0.sh` wrapper ; `cli/src/domain/schema-version.ts` versioned-selection (highest stable+scaffoldable) + scaffoldable:false refusal exit 3 (deferred B.8.3.b guard ; empty-metas→legacy fallback for mobile-only) ; new `b8-14-flip.test.sh` 9 L1 + 1 L2 (real Kong-less scaffold) registered in the forge-ci array ; tripwires inverted in lockstep (b8-3 _005/_006, b8-14 _003, b8-15 _005 front-door, t5-3-1 _007 count excludes additive 2.0.0/). **migrate-flagship stays ADDITIVE forever** (1.0.0 adopters keep Kong ; b8-12 + b8-15 line-225 green ; frozen 1.0.0 base/snapshot byte-untouched). §VIII.2/Temporal NOT amended (B8O). Framework stays 0.4.0 pre-GA (### BREAKING). Scope: pgvector/zitadel/qwik fresh-init wiring out-of-brick (shipped additively B.8.5/7/9). Full 54-harness suite + verify 414/0 + linter 61/0 + cli vitest 87/87 ; independent review `wf_63f4ec3f-0ca` 3 lanes re-executed live → APPROVE 0 must-fix ; archived 2026-06-05 ; gates the v0.4.0 stable cut) |
...
**53 archivés** au 2026-06-05 (zéro change en cours ; B.8.14 FLIP archivé 2026-06-05 — **B.8 COMPLETE 16/16**). Trio B.8.8 observability rearch
...
**B.8.12 archivé 2026-06-04** (E2E migration convergence gate, golden span superset + hermetic migrate-flagship driver + S2S Connect client + Envoy-OIDC wiring, see §4.2), **B.8.13 archivé 2026-06-04** (rollback runbook docs/ROLLBACK.md, record-only supersession of t4-pinned ARCHITECTURE-TARGET §11/§12.1, see §4.2), **B.8.14 PREPARE archivé 2026-06-04** (`b8-14-promotion-prep` — point-of-no-return prep, applies nothing breaking ; staged §VIII.1 Envoy amendment + Kong/REST removal manifest + flip runbook ; the §VIII.1 amendment is process-gated by a ≥7-day public discussion window so the ratify+apply+flip+remove is the follow-up `b8-14-promotion-flip`, see §4.2). **B.8.15 archivé 2026-06-04** (`b8-15-upgrade-matrix` — T5.1 Layer D forge-upgrade matrix gate, see §4.2). **B.8.14 FLIP archivé 2026-06-05** (`b8-14-promotion-flip` — THE point of no return, 2 ordered commits : C1 ratification `818ba6b` + C2 enablement ; Constitution **v2.0.0** §VIII.1 Kong→Envoy ratified (BDFL window waiver recorded honestly), 2.0.0 schema promoted `scaffoldable:true`, fresh `forge init --archetype full-stack-monorepo` now yields the **Kong-less Envoy/Connect tree** via `scaffold-plan-2.0.0.yaml` + the CLI versioned-selection guard, migrate-flagship stays additive (1.0.0 adopters keep Kong) ; independent review APPROVE ; see §4.2). **🎉 B.8 is COMPLETE (16/16).** Next: the **v0.4.0 stable cut** (B.8.15 is its gate) — at maintainer discretion. T5.3.1 livré sans `task validate` GREEN
...
## 0.14 Status update — 2026-09-08 (T8 — B.9 3/11 briques archivées ; état de release)

**T8 a démarré le 2026-07-27**, en avance sur le calendrier 2027-Q1 affiché en §11.
Trois briques de B.9 sont livrées, revues par lane indépendante et archivées le
2026-09-08. C'est le premier status-update B.9 : le précédent (§0.13) date du
2026-07-12, antérieur aux trois briques.

### Briques livrées

- **B.9.1 — `b9-1-schema`** (implémentée 2026-07-27). Schéma
  `.forge/schemas/mobile-pwa-first/2.0.0.yaml` en `stage: candidate`. Le point
  structurant est **ADR-B9-1-001**, ratifié par le mainteneur : le validateur
  versionné de B.8.3.b exigeait en dur les couches `{backend, frontend, infra}`,
  or cet archétype est *client-only*. Plutôt que de fabriquer des couches factices,
  le validateur a gagné un discriminateur `layer_profile: multi-layer | client-only`
  **défaillant à `multi-layer`** — les trois schémas versionnés existants gardent
  donc leur sens sans être édités.
- **B.9.2 — `b9-2-web-pwa`** (2026-07-28). Surface `web-pwa/` Qwik City + Service
  Worker + Web Push (VAPID) + manifest + coquille offline, portée sur un
  scaffold-plan. **ADR-B9-2-004 renverse la proposition** : un nouveau standard
  `pwa.yaml` plutôt qu'une extension de `web-frontend.yaml`, parce que ce dernier
  est un standard de *sélection de framework* et que la capacité PWA y est
  orthogonale. Cette brique enregistre aussi la clé de dispatch `mobile-pwa-first`,
  ce qui fait passer le refus de `forge init` de **exit 2 à exit 3** et force le
  basculement de trois assertions couplées de b9-1 dans le même change.
- **B.9.3 — `b9-3-shared-oidc`** (2026-07-31). Client OIDC navigateur sur
  `oauth4webapi` + configuration provider partagée. **Recadrage de la puce §5.2**
  (voir la note de correction qui y figure) : le plan disait « client TypeScript
  Connect-ES », ce qui est une erreur de catégorie — Connect-ES est un transport RPC
  vers un backend que cet archétype n'a pas. **ADR-B9-3-001/003 renversent** aussi la
  proposition « hand-rolled » vers `oauth4webapi`, un client OIDC correct devant
  valider la signature de l'ID token.

### Discipline de revue — ce qu'elle a réellement attrapé

Aucune brique n'a été auto-approuvée (Article V) : b9-1 et b9-2 signées
`APPROVE-WITH-NITS`, b9-3 `APPROVED` après **deux lanes indépendantes ayant toutes
deux rendu CHANGES REQUIRED**. Deux enseignements dépassent B.9 :

- **`pipefail` + `printf | grep -q` renvoie 141 sur un *match*** dès que le flux
  dépasse le tampon de 64 KiB. Quatre négatifs de b9-3 et le négatif VAPID de b9-2
  étaient donc silencieusement morts — un test qui ne pouvait pas échouer passait
  pour une preuve. Corrigés en here-strings, puis chaque négatif prouvé par mutation.
  **~40 autres harnesses partagent la forme et restent latents** (voir §0.14 « reste
  à faire »). Corollaire enregistré : le bug `[^\n]` n'était pas CI-only — le shim
  grep est propre au zsh interactif, le harness est bash et a toujours eu GNU grep.
- **Un résultat rouge qui ne prouve rien ressemble exactement à un rouge qui prouve
  tout.** La fixture écrite *parce que* T-L2-003 n'avait jamais tourné ne pouvait
...
### B.9 — COMPLÈTE, 11/11 (2026-09-12)

Toutes les briques sont livrées. `mobile-pwa-first / 2.0.0` est `stage: stable` /
`scaffoldable: true` depuis B.9.11 ; `forge init --archetype mobile-pwa-first` rend les
deux surfaces. **B.9.6 est un no-op** (linter NSMA déjà activé repo-wide par B.8.11).

Trois écarts mesurés restent ouverts et ne sont planifiés nulle part — voir
`b9-11-promotion-gate` Q-004 : `framework-owned-paths.yml` ne couvre pas `web-pwa/`
(b9-10 Q-001), le pin `flutter_bloc` diverge du standard (b9-5 Q-001), et
`forbidden_archetypes` est du code mort (b9-4 Q-001).
...
excluant les `candidate` : la bascule de promotion fait donc **réclamer la ligne
`mobile-pwa-first`** dans `docs/ARCHETYPES.md` (simulé et vérifié). Et `T-028` de
`b9-2.test.sh` épingle la mention `candidate` / `B.9.11` de `MIGRATION-PATHS.md`, qui
deviendra fausse à la promotion (b9-10 Q-003).
...
`web-pwa-ci.yml` : `dorny/paths-filter@v3` sur `web-pwa/**`, Node lu depuis
`web-pwa/.nvmrc`, puis analyse statique → build → `verify.sh` →
`constitution-linter.sh`, et upload de `dist/`. **Le job `pwa-deploy` prévu par
§5.2 n'est PAS livré** — arbitrage du mainteneur : un deploy scaffoldé exige des
secrets adoptant dès le premier push, donc rouge au jour 1, et `PWA-RULE-003`
place déjà ces secrets chez l'adoptant. Découverte au passage : c'est **la
première CI Qwik du dépôt** — la surface `frontend/web-public/` du flagship 2.0.0
n'en a pas non plus, et l'arbre 2.0.0 du flagship n'a aucun workflow (Q-001,
brique flagship).

Quatre manques que §5.2 ne prévoit pas et qui doivent entrer dans le périmètre :
le **projet de référence `examples/`** (B.6 et B.7 en livrent chacun un) ; le **gel
de `mobile-only / 1.0.0`** exigé par `upgrade-policy.md` au moment où une version est
...
|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------|
| B.6       | `event-driven-eu`                                                                                                                                  | ARCHITECTURE-TARGET §3.3          | `XL`    | ✅ **COMPLET 2026-07-12 (10/10 briques)** — promu `stage: stable` / `scaffoldable: true` (PR #46). Détails §0.13.                    |
| B.7       | `ai-native-rag`                                                                                                                                    | ARCHITECTURE-TARGET §3.3          | `XL`    | ✅ **COMPLET 2026-06-23 (9/9 briques)** — promu `stage: stable` / `scaffoldable: true` (PR #33). Détails §0.12.                      |
...
| B.9       | Migration `mobile-only / 1.0.0` → `mobile-pwa-first / 2.0.0` (PWA Qwik + Bloc renforcé)                                                            | ARCHITECTURE-TARGET §6.3          | `L`     | 🚧 **EN COURS — 6/11 briques archivées 2026-09-10** (B.9.1/B.9.2/B.9.3/B.9.7/B.9.8/B.9.9). Reste 4 briques ; B.9.6 est un no-op. B.9.7 livrée sans le job `pwa-deploy` (arbitrage). Détails §0.14.        |
...
| T5.2 | **Anti-Hallucination Platform Verification** — checklist 3-axis dans `document-specialist.md` + cadence re-verification dans `standards-lifecycle.md` v1.1.0 + harness `t5-2.test.sh`. Process change pour empêcher Q-004 / Q-006-class bugs. | Q-006 (Workiva web-only platform mismatch découvert 2026-05-16) | `S`–`M` | ✅ **Done 2026-05-18** via `t5-2-platform-verification` — détails §0.2. Indépendant de T5.1 / T5.3. |
| T5.3 | **`t5-otel-dartastic-realign`** — substitution Workiva `opentelemetry` (web-only) → écosystème **Dartastic** (`dartastic_opentelemetry_api` + `flutterrific_opentelemetry`, all-platform). Bump `flutter/opentelemetry.md` v1.1.0 → v2.0.0 (breaking). Rewrite flagship Flutter frontend OTel + mobile-only OTel template. Applique la checklist T5.2.A. | Q-006 résolution + bug architectural mobile/desktop OTel | `XL` | ✅ **Done 2026-05-18** via `t5-otel-dartastic-realign` — détails §0.3. Livré dans la série v0.4.0-rc (rupture standard). |
| K.1       | Hermes-Async (event-driven)                                                                                                                        | ARCHITECTURE-TARGET §9.2          | `M`     | ✅ Done 2026-07-12 via `b6-4-hermes-async` (PR #42)                                                                                 |
| K.2       | Sibyl (AI/RAG) — *renommé depuis `Pythia` à l'implémentation : collision avec la persona Product-Analyst-Pythia déjà livrée*                        | ARCHITECTURE-TARGET §9.2          | `M`     | ✅ Done 2026-06-22 via `b7-pythia` (PR #29)                                                                                        |
...
- **B.8.13.** Critères de rollback documentés (ARCHITECTURE-TARGET §11.3) :
    ...
    - ~~DBOS Postgres saturé > 70 % CPU → fallback Temporal pour workflows lourds.~~
      **REMOVED per B8O** — moot: Temporal is the orchestrator (no DBOS leg to roll back to).
      Effort : `S`. ✅ **Done 2026-06-04 via `b8-13-rollback-runbook`** — NEW `docs/ROLLBACK.md`
      (2 scenarios × Detect/Decide/Execute/Verify/Re-attempt + last-resort `--rollback` ;
      relative thresholds only, NO committed p99 per III.4 ; byte-consistent with
      `forge-migrate-flagship.sh:394-396`). **Record-only supersession**: ARCHITECTURE-TARGET.md
      is sha256-pinned by `t4.test.sh::_test_t4_023` (independent-review BLOCKER) ⇒ left
      byte-frozen ; the Supersession note enumerates the 7 B8O-stale DBOS refs
      (§11.1/§11.2 ×3/§11.3/§11.4 + §12.1:930 `default: dbos`) as obsolete-per-B8O →
      `orchestration.yaml` v1.2.0. `b8-13.test.sh` 18 L1 (positive arch-doc-sha-frozen guard
      + t4/b8-12/b8-10 coupling) ; design review round 1 CHANGES REQUIRED (t4-pin + 2 missed
...
- **B.8.15.** **T5.1 couche D — `forge upgrade` matrix test.** Test e2e matriciel
  ...
  ✅ **Done 2026-06-04 via `b8-15-upgrade-matrix`** — `b8-15.test.sh` realises the
  matrix as harness cells (forge-ci has no GA matrix). DELIVERED CELLS: cross-major
  `forge-upgrade.sh`→2.0.0 ⇒ exit 7 + `[NEEDS MIGRATION:]` (e2e binary) ; `--force`
  dirty same-major ⇒ refused ; migrate-flagship `--dry-run` on a c1 copy ⇒
  1.0.0→2.0.0 plan + no-op ; static additive no-`rm` guard ; flip-gated skip-pass
  guard (front-door 2.0.0 + Kong removal — fails-loud once 2.0.0 promotes). L2
  `FORGE_B8_15_LIVE`: real overlay on c1 ⇒ ledger flagship entry (1.0.0→2.0.0 +
  `kind`) + Kong-present + T5.1.B fixture matrix. ADR-B815-001: positive flagship
  drives `bin/forge-migrate-flagship.sh`, NOT the schema-resolving front door (the
  `1.0.0→2.0.0` front-door cell + the `mobile-pwa-first`/T7 cells remain FLIP/T7-gated).
  Same-major/ledger/.merge-conflicts via a7 coupling (no fixture dup). 6-mapper+critic
  workflow grounding ; design review CHANGES REQUIRED (1.0.0.tar.gz is the framework
  snapshot not a scaffolded project ; L1-dry-run can't assert a tree) → fixed (c1
  base + L1-static/L2-tree split) ; impl review APPROVE (L1 9/9 + L2 10/10,
  injection-tested additive guard). **B.8.15 is the v0.4.0-stable gate** — the
  `1.0.0→2.0.0` front-door + removal cells activate with the B.8.14 flip; the B.9/T7
  archetype cells are added when those ship.
...
- **B.9.3.** Templates partagés OIDC : le `AuthGateway` actuel (Flutter) est doublé
  ...
  > (→ ADR-B9-3-004).
- **B.9.4.** ✅ **Livrée 2026-09-11** via `b9-4-archetype-decision-tree`. L'arbre est
  livré tel que §5.2 le décrit, avec une branche que la formulation d'origine omettait :
  **iOS sans push critique prend la PWA comme partout ailleurs**.

  Ce que l'audit a trouvé en cherchant ce qui fonde la règle : la *conséquence de
  routage* est répétée à quatre endroits (`pwa.yaml:102-107`, schéma `2.0.0.yaml`
  ×2, `README.md.tmpl`, `index.tsx.tmpl`) et la **contrainte de plateforme nulle
  part** — aucun plancher de version iOS, aucune précondition d'installation sur
  l'écran d'accueil, aucune limite nommée de la Push API. Source unique :
  `ARCHITECTURE-TARGET.md:142`, une URL de blog externe consultée en 2026-04. La
  section documente donc la route et dit explicitement qu'elle **ne prétend rien sur
  la capacité d'iOS**, reprenant la formulation prudente de `pwa.yaml` (*« not
  assumed »*) plutôt que de l'aplatir en absolu — faux depuis Safari 16.4 (→ Q-002).

  Trois cellules de la matrice envoyaient le lecteur au mauvais endroit.
  `flutter-firebase` annonçait « Planned (B.2) » pour un archétype retiré de la
  taxonomie en 2026-05 (ADR-007), **et le refus `J8-RULE-001` que le même fichier
  promet 63 lignes plus bas ne se déclenche pas** : `parseDispatchTable` ne lit jamais
  `forbidden_archetypes`, un run réel sort en **127** avec une erreur bash (→ Q-001).
  `mobile-only` disait « Active » sans divulguer `status: legacy_alias`. Et la cellule
  Stack du flagship nommait encore **Kong** (zéro occurrence dans le plan fresh-init
  2.0.0) et promettait le swap **Temporal → DBOS annulé** pour Rust par ADR-B8O-001.

  **Aucune ligne `mobile-pwa-first` n'a été ajoutée**, délibérément (`ADR-B94-001`) :
  l'archétype est `candidate`, `forge init` sort en 3, et aucun archétype de ce dépôt
  n'a jamais eu de ligne pendant qu'il était candidate. À la place, le garde a été
  réécrit pour la **réclamer automatiquement à la promotion**.

  Ce garde était l'autre trouvaille : celui de `b5.test.sh` bouclait sur cinq noms
  figés d'avril 2026 et cherchait dans **tout** le document — **4 des 7 lignes
  supprimables CI verte**, `full-stack-monorepo` comprise, tandis que les deux qu'il
  épinglait étaient un archétype retiré et un non commencé. Dérivé de
  `dispatch-table.yml` et restreint aux lignes du tableau : **7/7 attrapées**.

  L'audit était adversarial et l'a mérité : **six trouvailles vérifiées sur neuf ont
...
- **B.9.7.** ✅ **Livrée 2026-09-09** via `b9-7-web-ci`, avec un périmètre élargi et
  une amputation, toutes deux assumées. Élargi : le besoin réel n'était pas « ajouter
  un job de déploiement » mais **toute la couche CI web** — l'archétype ne déclarait
  que le workflow B.4 `mobile-only` (`ios`/`android`/`e2e-android`/`summary`, filtré
  sur `lib|ios|android|pubspec`), si bien qu'une PR réécrivant toute la surface PWA ne
  déclenchait rien. Amputé : **pas de job `pwa-deploy`** — arbitrage du mainteneur, un
  deploy scaffoldé est rouge au jour 1 tant que l'adoptant n'a pas ses secrets, et
  `pwa.yaml::PWA-RULE-003` les lui attribue déjà ; le build est publié en artefact.
  Effort réel : `M`.
- **B.9.8.** ✅ **Livrée 2026-09-10** via `b9-8-snapshot`. Le snapshot
  `mobile-pwa-first/2.0.0.tar.gz` (553 fichiers) **plus** le gel de
  `mobile-only/1.0.0` qu'ADR-B8-2-004 avait explicitement renvoyé à B.9 et que
  §5.2 ne mentionnait pas. Ce dernier n'avait aucun `.sha256` et portait **299
  entrées AppleDouble sur 598** ; repacké (pas reconstruit — 50 de ses 219
  fichiers réels diffèrent de l'arbre actuel et portent l'état d'époque qui en
  fait une BASE de merge valide), puis gelé avec manifeste et garde de dérive.
  Effort réel : `M`.
- **B.9.9.** ✅ **Livrée 2026-09-10** via `b9-9-migrate-mobile-pwa`. Contrat mesuré
  avant d'être utilisé : rendre les deux archétypes depuis les mêmes entrées et
  differ donne **26 ajouts, 0 modification** — ADR-B9-1-004 confirmée. D'où l'absence
  de moteur de merge 3-way (contrairement au flagship) : refus sur collision plutôt
  que fusion. Valeurs de substitution **dérivées du projet** (`pubspec.yaml`,
...
- **B.9.11.** ✅ **Livrée 2026-09-12** via `b9-11-promotion-gate`. §5.2 ne décrivait
  que le harness ; §0.14 portait l'autre moitié, et c'est elle qui coûte : la bascule
  `candidate` → `stable` casse tous les frères qui assertent `candidate`.

  **Il n'existe aucun état vert intermédiaire.**
  `cli/test/e2e/archetypes-smoke.test.ts` partitionne les archétypes sur le champ
  `status` du dispatch — `scaffoldable = status !== "candidate"` (exige une fixture de
  confiance) et `candidates = status === "candidate"` (asserte un refus exit 3). Un
  schéma scaffoldable laissé à `status: candidate` est incohérent. `b7-6` avait livré
  exactement ce demi-état pour `ai-native-rag` et avait eu besoin d'un commit de
  réparation (`d21dda1`). Schéma, statut de dispatch, fixture CLI et six gardes frères
  basculent donc ensemble, et `b9.test.sh` T-012..T-015 assertent l'accord
  schéma↔dispatch **dans les deux sens**.

  Nouveau `b9.test.sh` : 28 L1 + 2 L2, enregistré en CI. Il asserte l'**état** promu
  comme invariant permanent, pas l'événement de promotion (`ADR-B911-002`). Sept sondes
  de mutation, 7/7 RED.

  Les six gardes tenus ont été **inversés, jamais supprimés** (`ADR-B911-001`). Le
  wrapper n'a demandé **aucune** édition : son garde lit `stage` et `scaffoldable` dans
  le schéma, et T-016 le prouve en rendant **sans** l'override de harness.

  **Deux pièges plantés par des briques antérieures se sont déclenchés seuls** — c'est
  précisément pourquoi ils avaient été plantés. `b5.test.sh` (réécrit par B.9.4) a
  réclamé la ligne de matrice à la seconde où le statut a basculé ; `T-028` (B.9.10,
  Q-003) est tombé sur l'aiguille `candidate`.

  **Et le job Vitest du CLI a attrapé ce que 81 harnesses shell verts n'ont pas vu** :
  la fixture listait un chemin Kotlin portant le domaine inverse de *ma* sonde, alors
  que le smoke test rend avec `--org dev.forge.test`. T-021 passait parce qu'il rendait
  avec le même domaine — deux artefacts d'accord sur une mauvaise réponse parce qu'ils
  partageaient une mauvaise entrée. C'est l'échec contre lequel `d21dda1` mettait en
  garde, trouvé parce que l'avertissement avait été lu d'abord.

  `forge-ci.yml` passe de 419 à **420/420** — le budget réservé à cet enregistrement
  est désormais dépensé (Q-002). Effort réel : `M`.
